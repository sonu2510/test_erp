<?php
class WalkInsamplesheet extends dbclass{
	
	public function addSample($data){
	
		$sql = "INSERT INTO `" . DB_PREFIX . "walk_in_sample_sheet` SET  customer_name = '" .$data['customer_name']. "',customer_visit_date = '".$data['customer_visit_date']."',customer_address = '".$data['customer_address']."',email ='".$data['email']."',telephone = '".$data['telephone']."' ,customer_requirements = '".$data['customer_requirements']."',customer_Product = '".$data['customer_Product']."',weight = '".$data['weight']."',total_bag = '".$data['total_bag']."',sample_name = '".$data['sample_name']."',attend_customer = '".$data['attend_customer']."',result ='".$data['result']."',f1_date ='".$data['f1_date']."',f1_description ='".$data['f1_description']."',f2_date ='".$data['f2_date']."',f2_description ='".$data['f2_description']."',f3_date = '" .$data['f3_date']. "',f3_description ='".$data['f3_description']."',deal ='".$data['deal']."',status = '" .$data['status']. "',date_added = NOW(),date_modify = NOW(),is_delete=0";
	...
	public function updateSample($sample_id,$data){
		...
		$sql = "UPDATE `" . DB_PREFIX . "walk_in_sample_sheet` SET  customer_name = '" .$data['customer_name']. "',customer_visit_date = '".$data['customer_visit_date']."',customer_address = '".$data['customer_address']."',email ='".$data['email']."',telephone = '".$data['telephone']."',customer_requirements = '".$data['customer_requirements']."',customer_Product = '".$data['customer_Product']."',weight = '".$data['weight']."',total_bag = '".$data['total_bag']."',sample_name = '".$data['sample_name']."',attend_customer = '".$data['attend_customer']."',result ='".$data['result']."',f1_date ='".$data['f1_date']."',f1_description ='".$data['f1_description']."',f2_date ='".$data['f2_date']."',f2_description ='".$data['f2_description']."',f3_date = '" .$data['f3_date']. "',f3_description ='".$data['f3_description']."',deal ='".$data['deal']."',status = '" .$data['status']. "',  date_modify = NOW() WHERE sample_id = '" .(int)$sample_id. "'";
		$this->query($sql);		
	}
	
	public function getTotalSample($filter_data=array()){
		$sql = "SELECT COUNT(*) as total FROM `" . DB_PREFIX . "walk_in_sample_sheet` WHERE is_delete = 0";
	...
	public function getSamples($data,$filter_data=array()){
		$sql = "SELECT * FROM `" . DB_PREFIX . "walk_in_sample_sheet` WHERE is_delete = 0";
	...
	public function getSample($sample_id){
		$sql = "SELECT * FROM `" . DB_PREFIX . "walk_in_sample_sheet` WHERE sample_id = '" .(int)$sample_id. "'";
		$data = $this->query($sql);
		if($data->num_rows){
			return $data->row;
		}else{
	...
	public function updateStatus($id,$status){
		$sql = "UPDATE `" . DB_PREFIX . "walk_in_sample_sheet` SET status = '" .(int)$status. "',  date_modify = NOW() WHERE 
		sample_id = '".$id."' ";
		//echo $sql;die;
	    $this->query($sql);
	}
		
	public function updateSampleStatus($status,$data){//printr($data);
		if($status == 0 || $status == 1){
			$sql = "UPDATE `" . DB_PREFIX . "walk_in_sample_sheet` SET status = '" .(int)$status. "',  date_modify = NOW() WHERE sample_id IN (" .implode(",",$data). ")";
			$this->query($sql);
		}elseif($status == 2){
			$sql = "UPDATE `" . DB_PREFIX . "walk_in_sample_sheet` SET is_delete = '1', date_modify = NOW() WHERE sample_id IN (" .implode(",",$data). ")";
			$this->query($sql);
		}
	}
	...
	/*public function getCountry()
		...
		{
			$data = $this->query("SELECT * FROM `walk_in_sample_sheet` WHERE sample_id = '".$sample_id."' AND status = '1' AND is_delete = 0 ");
			if($data->num_rows)
			{
				return $data->row;
			}
			...
			$html.="<div class='form-group'>
					    ...
					    <table class='table table-striped b-t text-small' id='stock_order' >
                                ...
                                $html .=" <tr  valign='top'><td>Customer Name: ".$sample_detail['customer_name']." <br><br> Customer Visit Date: ".dateFormat(4,$sample_detail['customer_visit_date'])." <br><br> Address: ".$sample_detail['customer_address']." <br><br> Email: ".$sample_detail['email']." <br><br> Telephone: ".$sample_detail['telephone']." </td>
                                 ...
                                 <td>".$sample_detail['sample_name']."</td>
								 
                                 <td>".$sample_detail['attend_customer']."</td>
								 
                                 <td>".$sample_detail['result']."</td>

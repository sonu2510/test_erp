...
class lamination extends dbclass{
	...
	//	printr($data);die;
		...
		if(isset($data['lamination_id'])){
		    ...
		    $name=explode('==',$layers_details['product_item_layer_id']);
				...
				if($data['printing_status']=='1'){
					...
					if(!isset($data['lamination_id'])){
								...
								if(!empty($printing_details)){  
									...
									$this->query("INSERT INTO  lamination_layer SET lamination_id ='".$lamination_id."',layer_no ='1',job_id='".$data['job_id']."',roll_used='".$printing_details['f_roll_used']."',product_item_layer_id='".$printing_details['f_product_item_layer_id']."',printing_status='".$printing_details['printing_status']."', total_output = '0',operator_id = '" .$printing_details['f_operator_id']. "',junior_id = '" .$printing_details['f_junior_id']. "',input_qty = '" .$printing_details['f_input_qty']. "',plain_wastage = '0',layer_date='".$data['lamination_date']."',print_wastage = '0',total_wastage = '0',wastage_per='0', date_added = NOW(),date_modify = NOW(),is_delete=0");
										$lamination_layer_id = $this->getLastlamination_layerId();
							
							//printing detail first layer
						...
						//second layer entry
						$sql_l="INSERT INTO  lamination_layer SET lamination_id ='".$lamination_id."',layer_no ='".$layers_details['layers']."',adhesive_grade ='".$layers_details['adhesive_grade']."',product_item_layer_id ='".$name[0]."',job_id='".$data['job_id']."',input_qty='".$layers_details['input_qty']."',output_qty='".$layers_details['output_qty']."',balance_qty='".$layers_details['balance_qty']."',output_qty_m='".$layers_details['output_qty_m']."',roll_used='".$layers_details['roll_used']."',film_size='".$layers_details['film_size']."',operator_id='".$layers_details['operator_id']."',junior_id='".$layers_details['junior_id']."',operator_shift='".$layers_details['operator_shift']."',layer_date='".$layers_details['layer_date']."',plain_wastage = '" .$layers_details['plain_wastage']. "',print_wastage = '" .$layers_details['print_wastage']. "',total_wastage = '" .$layers_details['total_wastage']. "',wastage_per='".$layers_details['wastage_per']."',printing_status='0' , date_added = NOW(),date_modify = NOW(),is_delete=0";
	
						$data = $this->query($sql_l);
					$lamination_layer_id = $this->getLastlamination_layerId();
				
	
						
					}else{
					    //direct job all layer

						$this->query("UPDATE job_master SET lamination_status='1' WHERE job_id='".$data['job_id']."'");
				    
				    	$sql="INSERT INTO  lamination_layer SET lamination_id ='".$lamination_id."',layer_no ='".$layers_details['layers']."',adhesive_grade ='".$layers_details['adhesive_grade']."',product_item_layer_id ='".$name[0]."',job_id='".$data['job_id']."',input_qty='".$layers_details['input_qty']."',output_qty='".$layers_details['output_qty']."',balance_qty='".$layers_details['balance_qty']."',output_qty_m='".$layers_details['output_qty_m']."',roll_used='".$layers_details['roll_used']."',film_size='".$layers_details['film_size']."',operator_id='".$layers_details['operator_id']."',junior_id='".$layers_details['junior_id']."',operator_shift='".$layers_details['operator_shift']."',layer_date='".$layers_details['layer_date']."',plain_wastage = '" .$layers_details['plain_wastage']. "',print_wastage = '" .$layers_details['print_wastage']. "',total_wastage = '" .$layers_details['total_wastage']. "',wastage_per='".$layers_details['wastage_per']."',printing_status='0' , date_added = NOW(),date_modify = NOW(),is_delete=0";
...
//die;
	...
	{
		$sql = "SELECT lamination_no FROM `" . DB_PREFIX . "lamination` WHERE is_delete = 0 ORDER BY  lamination_no DESC";
		$data = $this->query($sql);
		if($data->num_rows){
			return $data->row['lamination_no'];
		}else{
			return false;
		}
	}
	public function getTotalLamination($filter_data=array(),$job_date='')
	{
		...
		$sql = "SELECT COUNT(*) as total FROM  lamination as pb, job_master as jm ,machine_master as mm WHERE pb.is_delete = 0 AND pb.machine_id = mm.machine_id AND    pb.job_id=jm.job_id $con ";
		//printr($filter_data);
	...
	public function getLamination($option,$filter_data=array(),$job_date='')
	{
		...
		$sql = "SELECT pb.*,jm.job_name,mm.machine_name FROM  lamination as pb, employee as om, job_master as jm,machine_master as mm WHERE pb.is_delete = 0 AND pb.machine_id = mm.machine_id AND   pb.job_id=jm.job_id $con GROUP 	by lamination_id ";
...
/*	public function getPrintting_details($job_id)
	...
	*/
	...
	public function getLayerJObMaterialName($product_item_id){
		$sql ="SELECT * FROM  product_item_info WHERE  product_item_id ='".$product_item_id."' AND is_delete='0' AND status='1'";
		$data = $this->query($sql);
			//printr($data);
			if ($data->num_rows) {
		...
		$html = '';
					...
					$html.='</div>
						...
						$html.='	<table class="table">
							...
							<td style="vertical-align: top;width: 50%;">
								...
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                              <b>Shift :</b>   '.ucwords($lamination_details['shift']).'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
							</td>
							<td style="vertical-align: top;width: 50%;">
				...
				$html .='<div class="div_first" style="width: 100%; float: left;  ">';
					...
					$html.='	<table class="table" style="font-size: 14px;">
								<tbody>
								<tr>
									<td width="10%"><div align="center"><b>Layer No.</b></div></td>
									<td width="10%"><div align="center"><b>Operator Details</b></div></td>
									<td width="10%"><div align="center"><b>Roll Used</b></div></td>
									';
							
							$html.='
									
								
									<td width="15%"><div align="center"><b> I/P(Kgs)</b></div></td>
									<td width="15%"><div align="center"><b> O/P(Kgs)</b></div></td>
									<td width="10%"><div align="center"><b>Balance Qty(Kgs)</b></div></td>
								...
								foreach($layer_details as $layer){
								 //   printr($layer);
									...
									if( $layer['input_qty']!=0.000){
										$layer['input_qty']=$layer['input_qty'].'(kgs)';
								
									}else{
										$layer['input_qty']='-';
									}
									if( $layer['output_qty']!=0.000){
								
									$layer['output_qty']=$layer['output_qty'].'(kgs)';
										
									}else{
										$layer['output_qty']='-';
									}
									if( $layer['output_qty_m']!=0.000){
									 
									$layer['output_qty_m']=$layer['output_qty_m'].'(kgs)';
										
									}else{
										$layer['output_qty_m']='-';
									}	
									if( $layer['balance_qty']!=0.000){
									
									$layer['balance_qty']=$layer['balance_qty'].'(kgs)';
									...
									<td width="10%"><div align="center">'.$operator_name.' <b>'.$layer['operator_shift'].'</b></div></td>
									<td width="5%"><div align="center">'.$layer['roll_used'].'</div></td>
									<td width="10%"><div align="center">'. $layer['input_qty'].'</div></td>
									<td width="10%"><div align="center">'. $layer['output_qty'].'</div></td>
								    <td width="10%"><div align="center">'.$layer['balance_qty'].'</div></td>

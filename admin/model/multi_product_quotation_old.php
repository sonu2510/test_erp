			...
			if($type=='Q')
			{
				...
				else
				{
					...
					{	
							...
							{
									$sql2 = "UPDATE company_address SET country = '" . $data['country_id']. "',email_1 = '" . addslashes($data['email']) . "' WHERE company_address_id ='" . $data['company_address_id'] . "'";
									$datasql2 = $this->query($sql2);
						...
						$spout_additional_packing_price=$spout_by_sea_price['additional_packaging_price'];
							...
							if(isset($data['country_id']) && !empty($data['country_id']) && $data['country_id'] ==111)
							{
								...
								else
								{
									...
									$final_tax_name=$data['normalform'];
								}
								if($tax_name!='')
								{
					...
					if($type=='Q')
					{
						...
						if(isset($taxation_data) && !empty($taxation_data))
						{
							...
							if($data['normalform']=='Out Of Gujarat')
							{
								...
								$test['$cylinderpriceFortax+($cylinderpriceFortax*$taxation_data[igst]/100)']=$cylinderpriceFortax.'+('.$cylinderpriceFortax.'*'.$taxation_data['igst'].'/100)';
								
								$test['$totaltoolpriceWithTax = $toolpriceFortax+($toolpriceFortax*$taxation_data[igst]/100);']=$toolpriceFortax.'+('.$toolpriceFortax.'*'.$taxation_data['igst'].'/100)';
							...
							else
							{
								...
								//$test['$cylinderpriceFortax+($cylinderpriceFortax*($taxation_data[icst]+$taxation_data[scst])/100)']=$cylinderpriceFortax.'+('.$cylinderpriceFortax.'*('.$taxation_data['cgst'].'+'.$taxation_data['sgst'].')/100)';
								//$test['$toolpriceFortax+($toolpriceFortax*($taxation_data[icst]+$taxation_data[scst])/100)']=$toolpriceFortax.'+('.$toolpriceFortax.'*('.$taxation_data['cgst'].'+'.$taxation_data['sgst'].')/100)';
							}
													...
													$pricesql = "INSERT INTO ".DB_PREFIX."multi_product_quotation_price SET product_quotation_id = '".$productQuatiationId."', product_quotation_quantity_id = '".$productQuatiationQuantityId."', zipper_txt = '".$zipData['zip_text']."',gusset_printing_type='".$data['gusset_printing_option']."',valve_txt = '".$zipData['valve_text']."', date_added = NOW(), zipper_price = '".$zipData['calculateZipperPrice']."', spout_txt = '".$zipData['spout_txt']."', spout_base_price = '".$zipData['spout_price']."', accessorie_txt = '".$zipData['accessorie_txt']."', make_pouch = '".(int)$data['make']."',spout_pouch_type='".$zipData['spout_pouch_type']."',accessorie_base_price = '".$zipData['accessorie_price']."',valve_price = '".$zipData['valve_price']."' , accessorie_txt_corner = '".$zipData['accessorie_txt_corner']."', accessorie_base_price_corner = '".$zipData['accessorie_price_corner']."',actual_laser_price='".$laser_price."',laser_id='".$data['laser_name']."',handle_price = '".$handle_price."',handle_id='".$handle_id."',cyli_gress_price='".$cylinderCurrencyGressPrice."',gress_cyli_per='".$userInfo['gres_cyli']."',tamperevident='".$tamperevident."',slider_place='".$slider_place."',";
														...
														if(isset($taxation_data) && !empty($taxation_data))
														{
															...
															else
															{
																...
																$test['$totalPricWithExcies+($totalPricWithExcies*($taxation_data[cgst]+$taxation_data[sgst])/100)']=$totalPricWithExcies.'+('.$totalPricWithExcies.'*('.$taxation_data['cgst'].'+'.$taxation_data['sgst'].')/100)';
		...
		if($addedByInfo)
		{
			...
			foreach($email_temp as $val)
			{
				...
				if($val['html'])
				{
				    ...
				    //if($_SESSION['ADMIN_LOGIN_SWISS']!='238'  && $_SESSION['LOGIN_USER_TYPE']!='2')
				         send_email($val['email'],$formEmail,$subject,$message,'');
				   /* else
				        printr($message);printr($val['email']);*///send_email('erp@swisspac.net',$formEmail,$subject,$message,'');

<?php 
class maxicoTaxCalender extends dbclass
{
	public function addTax($post)
	{
		$sql = "INSERT INTO maxico_tax_calender SET  date = '".$post['date']."',description = '".$post['description']."',remainder_date = '".$post['remainder_date']."',last_remainder_date = '".$post['last_remainder_date']."',reminder = '".$post['reminder']."' ,status = '" .$post['status']. "',user_id = '".$_SESSION['ADMIN_LOGIN_SWISS']."',user_type_id = '".$_SESSION['LOGIN_USER_TYPE']."' ,date_added = NOW(),is_delete=0  "  ;
		...
		if(isset($post['img_nm']) && !empty($post['img_nm']))
		{
			foreach($post['img_nm'] as $imgnm)
				{
					//printr($imgnm);//die;
					$sql1 = "INSERT INTO maxico_image_upload SET  tax_calender_id = '".$last_id."',image_name = '".$imgnm."',user_id = '".$_SESSION['ADMIN_LOGIN_SWISS']."',user_type_id = '".$_SESSION['LOGIN_USER_TYPE']."',date_added = NOW(),is_delete=0";
					//echo $sql1;die;
	...
	public function updateTax($tax_calender_id,$post)
	{
		//printr($post);
		//die;
		$sql = "UPDATE maxico_tax_calender SET date = '".$post['date']."',description = '".$post['description']."',remainder_date = '".$post['remainder_date']."',last_remainder_date = '".$post['last_remainder_date']."',reminder = '".$post['reminder']."' ,status = '" .$post['status']. "', date_modify = NOW() WHERE tax_calender_id = '" .(int)$tax_calender_id. "'";
		...
		if(isset($post['img_nm']) && !empty($post['img_nm']))
		{
			foreach($post['img_nm'] as $imgnm)
			{
				if($imgnm!='')
				{
						$sql1 = "INSERT INTO maxico_image_upload SET  tax_calender_id = '".$tax_calender_id."',image_name = '".$imgnm."',user_id = '".$_SESSION['ADMIN_LOGIN_SWISS']."',user_type_id = '".$_SESSION['LOGIN_USER_TYPE']."',date_added = NOW(),is_delete=0";
						$data1 = $this->query($sql1);	
				}
			}
		}		
	...
	public function updateStatus($status,$post){
		if($status == 0 || $status == 1){
			$sql = "UPDATE maxico_tax_calender SET status = '" .(int)$status. "',  date_modify = NOW() WHERE tax_calender_id IN (" .implode(",",$post). ")";
			$this->query($sql);
		}elseif($status == 2){
			$sql = "UPDATE maxico_tax_calender SET is_delete = '1', date_modify = NOW() WHERE tax_calender_id IN (" .implode(",",$post). ")";
			$this->query($sql);
		}
	}

	public function getTotalTax(){
		$sql = "SELECT COUNT(*) as total FROM maxico_tax_calender WHERE is_delete=0";
		//echo $sql;die; 
		$data = $this->query($sql);
		if($data->num_rows){
	...
	public function getTax($data){
		$sql = "SELECT * FROM maxico_tax_calender WHERE is_delete=0";
		
		if (isset($data['sort'])) {
			$sql .= " ORDER BY " . $data['sort'];	
		} else {
			$sql .= " ORDER BY tax_calender_id";	
		}
	...
	public function getTaxRecord($tax_calender_id)
	{
		$sql = "SELECT * FROM maxico_tax_calender WHERE is_delete=0 AND tax_calender_id= '".$tax_calender_id."'";
	...
	public function updateMaxicotax($tax_id,$status){
		$this->query("UPDATE maxico_tax_calender SET status = '" .(int)$status. "',  date_modify = NOW() WHERE tax_calender_id='".$tax_id."'");
		echo "UPDATE maxico_tax_calender SET status = '" .(int)$status. "',  date_modify = NOW() WHERE tax_calender_id='".$tax_id."'";
	}
	
	public function updateImagestatus($tax_id,$status)
	{
		$sql = "UPDATE maxico_tax_calender SET status = '".$status."', date_modify = NOW()  WHERE tax_id = '" .(int)$tax_id. "'";
		$this->query($sql);
	}
	...
	public function getTaxImgRecord($tax_calender_id)
	{
		$sql = "SELECT * FROM maxico_image_upload WHERE is_delete=0 AND tax_calender_id= '".$tax_calender_id."'";
	...
	public function removeAusUploadRecord($max_image_id)
	{
		$sql = "UPDATE maxico_image_upload SET is_delete = '1'  WHERE max_image_id = '" .(int)$max_image_id. "'";
		$this->query($sql);
		return 1;
	}
	
	public function getTodayReminderDatesMax(){
		date_default_timezone_set('UTC');
		$today_date = date("Y-m-d");
				 
		$data = $this->query("SELECT * FROM `" . DB_PREFIX . "maxico_tax_calender` WHERE reminder='1' AND status='1' AND is_delete='0' AND MONTH(date)= MONTH(CURDATE()) AND YEAR(date)=YEAR(CURDATE()) AND (remainder_date='".$today_date."' OR last_remainder_date='".$today_date."')");
		if($data->num_rows){
			return $data->rows;
		}else{
			return false;
	...
	public function sendEmail($tax_calender_id,$adminEmail)
	{
		$max = $this->getTaxRecord($tax_calender_id);
		if($max)
		{
			$max_msg = '';

<?php
class stockProfitBySea extends dbclass{
	
	public function addProfit($data){

		$sql = "INSERT INTO `" . DB_PREFIX . "stock_profit_by_sea` SET product_id = '".(int)$data['product_id']."', from_quantity = '" .(int)$data['from_quantity']. "', to_quantity = '" .(int)$data['to_quantity']. "', profit = '" . (float)$data['profit']. "',profit_poor = '".(float)$data['profit_poor']."',profit_more_poor = '".(float)$data['profit_more_poor']."',date_added = NOW()";
		$this->query($sql);
		return $this->getLastId();
	}
	public function updateProfit($product_id,$data){
		
		$this->query("DELETE FROM `" . DB_PREFIX . "stock_profit_by_sea` WHERE product_id='".$product_id."'");
	...
	public function getProfitPrices($product_id,$quantity_id){
		$sql = "SELECT * FROM `" . DB_PREFIX . "stock_profit_by_sea` WHERE product_id = '" .(int)$product_id. "' AND quantity_id='".$quantity_id."' ";
		$data = $this->query($sql);
	...
	public function getTotalProfit(){
		$sql = "SELECT COUNT(*) as total FROM `" . DB_PREFIX . "stock_profit_by_sea`";
		$data = $this->query($sql);
	...
	public function getProfits($data){
		$sql = "SELECT pp.*, p.product_name FROM " . DB_PREFIX . "stock_profit_by_sea pp INNER JOIN " . DB_PREFIX . "product p ON(pp.product_id = p.product_id) ";
		
		if (isset($data['sort'])) {
			$sql .= " ORDER BY " . $data['sort'];	
		} else {
			$sql .= " ORDER BY pp.stock_profit_by_sea_id";	
		}

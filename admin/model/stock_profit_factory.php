<?php
class stockProfitFactory extends dbclass{
	
	public function addProfit($data){

		$sql = "INSERT INTO `" . DB_PREFIX . "stock_profit_factory` SET product_id = '".(int)$data['product_id']."', from_quantity = '" .(int)$data['from_quantity']. "', to_quantity = '" .(int)$data['to_quantity']. "', profit = '" . (float)$data['profit']. "',profit_poor = '".(float)$data['profit_poor']."',profit_more_poor = '".(float)$data['profit_more_poor']."',date_added = NOW()";
		$this->query($sql);
	...
	public function getProfitPrices($product_id,$quantity_id){
		$sql = "SELECT * FROM `" . DB_PREFIX . "stock_profit_factory` WHERE product_id = '" .(int)$product_id. "' AND quantity_id='".$quantity_id."' ";
		$data = $this->query($sql);
	...
	public function getTotalProfit(){
		$sql = "SELECT COUNT(*) as total FROM `" . DB_PREFIX . "stock_profit_factory`";
		$data = $this->query($sql);
	...
	public function getProfits($data){
		$sql = "SELECT pp.*, p.product_name FROM " . DB_PREFIX . "stock_profit_factory pp INNER JOIN " . DB_PREFIX . "product p ON(pp.product_id = p.product_id) ";
		
		if (isset($data['sort'])) {
			$sql .= " ORDER BY " . $data['sort'];	
		} else {
			$sql .= " ORDER BY pp.stock_profit_factory_id";	
		}

<?php
class stockWastage extends dbclass{
	
	public function addWastage($data){
		$sql = "INSERT INTO `" . DB_PREFIX . "stock_wastage` SET from_quantity = '" .(int)$data['from_quantity']. "', to_quantity = '" .(int)$data['to_quantity']. "', wastage = '" .$data['wastage']. "', date_added = NOW()";
		$this->query($sql);
	...
	public function updateWastage($wastage_id,$data){
		//printr($data);
		$sql = "UPDATE `" . DB_PREFIX . "stock_wastage` SET from_quantity = '" .(int)$data['from_quantity']. "', to_quantity = '" .(int)$data['to_quantity']. "', wastage = '" .$data['wastage']. "', date_modify = NOW() WHERE stock_wastage_id = '" .(int)$wastage_id. "'";
		//echo "UPDATE `" . DB_PREFIX . "stock_wastage` SET from_quantity = '" .(int)$data['from_quantity']. "', to_quantity = '" .(int)$data['to_quantity']. "', wastage = '" .$data['wastage']. "', date_modify = NOW() WHERE stock_wastage_id = '" .(int)$wastage_id. "'";die;
		$this->query($sql);
	}
	
	public function getWastage($wastage_id){
		$sql = "SELECT * FROM `" . DB_PREFIX . "stock_wastage` WHERE stock_wastage_id = '" .(int)$wastage_id. "'";
		$data = $this->query($sql);
		if($data->num_rows){
			return $data->row;
		}else{
	...
	public function getProductOtherList($product_id){
			...
			foreach($product_id as $key=>$val)
			 ...
			 // die;
		$sql = "SELECT * FROM " . DB_PREFIX . "product WHERE product_id NOT IN($updated_string) AND is_delete = 0 AND status = 1";
		$data = $this->query($sql);
		if($data->num_rows){
			return $data->rows;
		}else{
	...
	public function getTotalWastage(){
		$sql = "SELECT COUNT(*) as total FROM `" . DB_PREFIX . "stock_wastage`";
		$data = $this->query($sql);

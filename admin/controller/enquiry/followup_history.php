      ...
      <div class="col-lg-12">
        <section class="panel">
          <header class="panel-heading bg-white"> 
          	...
          	<span>FollowUp History Listing</span>
          ...
          <div class="panel-body">
              <form class="form-horizontal" method="post" data-validate="parsley" action="<?php echo $obj_general->link($rout, 'mod=followup_history'.$add_url, '',1); ?>">
                  ...
                  <footer class="panel-footer <?php echo $class; ?>">
                    <div class="row">
                       <div class="col-lg-12">
                        ...
                        <a href="<?php echo $obj_general->link($rout, 'mod=followup_history'.$add_url, '',1); ?>" class="btn btn-info btn-sm pull-right" ><i class="fa fa-refresh"></i> Refresh</a>
                       </div> 
                    </div>
                  </footer>                                  
              </section>
              ...
              <div class="row">
  			    <div class="pull-left">
				    <div class="panel-body text-muted l-h-2x">
    					...
    					{ ?>
    							<a href="<?php echo $obj_general->link($rout, 'mod=followup_history&all_emp=1','',1); ?>" class="btn btn-primary btn-sm pull-right ml5" name="daywise" style="background-color:#CBC6AB"> All Leads</a>
    							<a href="<?php echo $obj_general->link($rout, 'mod=followup_history','',1); ?>" class="btn btn-primary btn-sm pull-right ml5" name="alljobs" style="background-color:#81C267">Your Leads</a>
                 ...
                 <div class="col-lg-3 pull-right">	
                     ...
                     <option value="<?php echo $obj_general->link($rout, 'mod=followup_history'.$add_url, '',1);?>" selected="selected">--Select--</option>
                        	<?php 
    							$limit_array = getLimit(); 
    							foreach($limit_array as $display_limit) {
    								//printr($display_limit);
    								if($limit == $display_limit) {	 
    						?>
                            	
                            		<option value="<?php echo $obj_general->link($rout, 'mod=followup_history'.'&limit='.$display_limit.$add_url, '',1);?>" selected="selected"><?php echo $display_limit; ?></option>				
    						<?php } else { ?>
                                	<option value="<?php echo $obj_general->link($rout, 'mod=followup_history'.'&limit='.$display_limit.$add_url, '',1);?>"><?php echo $display_limit; ?></option>
                            <?php } ?>
                            <?php } ?>
                     </select>
                 </div>
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  <thead>
                    <tr>
                      ...
                      <th class="th-sortable <?php echo ($sort_order=='ASC') ? 'active' : ''; ?> ">
                      		 Enquiry Number
                            <span class="th-sort">
                            	<a href="<?php echo $obj_general->link($rout, 'mod=followup_history'.'&sort=enquiry_number'.'&order=ASC '.$add_url, '',1);?>">
                                <i class="fa fa-sort-down text"></i>
                                <a href="<?php echo $obj_general->link($rout, 'mod=followup_history'.'&sort=enquiry_number'.'&order=DESC '.$add_url, '',1);?>">
                  ...
                  <tbody>
                  <?php 
                  $total_enquiry = $obj_enquiry->getTotalFollowup($filter_data,$address_id,$all_emp);
             //  printr($total_enquiry);die;
				  $pagination_data = '';
                  if($total_enquiry){
                        if (isset($_GET['page'])) {
                            $page = (int)$_GET['page'];
					  ...
					  $enquiries = $obj_enquiry->getFollowup($option,$filter_data,$address_id,$all_emp);
					  ...
					  foreach($enquiries as $enquiry){ 
                        ...
                        <tr>
                          ...
                          <td>
                           ...
                           <td><a href="<?php echo $obj_general->link($rout, 'mod=view&enquiry_id='.encode($enquiry['enquiry_id']).$add_url, '',1); ?>"><?php echo $enquiry['enquiry_note'];?></a></td>
                           <td><a href="<?php echo $obj_general->link($rout, 'mod=view&enquiry_id='.encode($enquiry['enquiry_id']).$add_url, '',1); ?>"><?php echo dateFormat(4,$enquiry['followup_date']);?></a></td>
                          ...
                          <td><a href="<?php echo $obj_general->link($rout, 'mod=view&enquiry_id='.encode($enquiry['enquiry_id']).$add_url, '',1);?>"><?php echo dateFormat(4,$enquiry['reminder']);?></a></td>                                           <td>
                        ...
                        $pagination->url = $obj_general->link($rout, '&page={page}&limit='.$limit.'&mod=followup_history'.$add_url.''.$all_url, '',1);

           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table table-striped b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php
				  $total = $obj_punching->getlabelPunchings();
                    $total_punch = count($total);
				  $pagination_data = '';
                  if($total_punch){
                        if (isset($_GET['page'])) {
                            $page = (int)$_GET['page'];
                      ...
                      $option = array(
                            'sort'  => 'label_punching_id',
                            'order' => 'ASC',
                            'start' => ($page - 1) * LISTING_LIMIT,
                            'limit' => LISTING_LIMIT
                      );	
                      $punchings = $obj_punching->getlabelPunchings($option);
					  foreach($punchings as $punch){ 
                        ?>
                        <tr>
                          <td><?php echo $punch['from_qty'];?></td>
                          <td><?php echo $punch['to_qty'];?></td>
                          <td><?php echo $punch['price'];?></td>
                          <td>
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&punching_id='.encode($punch['label_punching_id']), '',1); ;?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                           </td>
                        </tr>
                        <?php
                      }

...
if(isset($obj_session->data['filter_data'])){
	$filter_unit = $obj_session->data['filter_data']['unit'];
...
if($display_status) {
	...
	if(isset($_POST['action']) && ($_POST['action'] == "active" || $_POST['action'] == "inactive") && isset($_POST['post']) && !empty($_POST['post']))
	{
		...
		$obj_unit->updateStatus($status,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	
	}else if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post'])){
	 //printr($_POST['post']);die;
		$obj_unit->updateStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
                      ...
                      <div class="row">
                          <div class="col-lg-4">
                              <div class="form-group">
                                <label class="col-lg-5 control-label">Product unit</label>
                                <div class="col-lg-7">
                                  <input type="text" name="filter_unit" value="<?php echo isset($filter_unit) ? $filter_unit : '' ; ?>" placeholder="unit" id="input-name" class="form-control" />
                                </div>
                              </div>                             
                          </div>
                          <div class="col-lg-4">                              
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php	
                  $total_unit = $obj_unit->getTotalProduct($filter_data);
                  $pagination_data = '';
                  if($total_unit){
                        if (isset($_GET['page'])) {
                            $page = (int)$_GET['page'];
                      ...
                      $products = $obj_unit->getProducts($option,$filter_data);
					  
					  foreach($products as $product){ 
                        ?>
                        <tr <?php echo ($product['status']==0) ? 'style="background-color:#FADADF" ' : '' ; ?>>
                          <td><input type="checkbox" name="post[]" value="<?php echo $product['unit_id'];?>"></td>
                                    
                          <td><?php $unit = $product['unit'];  
					         echo $unit;?></td>
                          <td>
                          	<div data-toggle="buttons" class="btn-group">
                                <label class="btn btn-xs btn-success <?php echo ($product['status']==1) ? 'active' : '';?> "> <input type="radio" 
                                 name="status" value="1" id="<?php echo $product['unit_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label> 
                                                                   
                                <label class="btn btn-xs btn-danger <?php echo ($product['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                    name="status" value="0" id="<?php echo $product['unit_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label> 
                            </div>
                          </td>
                         <?php if($edit_per==1){ ?> <td>	
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&unit_id='.$product['unit_id'],'',1); ?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                           </td><?php } ?>
                        </tr>
                        <?php
                      }
...
<script type="application/javascript">
	$('input[name=status]').change(function() {
		...
		//alert(status_value);
		var status_url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&ajax=UpdateProductunitStatus', '',1);?>");
        //alert(status_url);
		$.ajax({			
			url : status_url,
			type :'post',
			data :{unit_id:unit_id,status_value:status_value},
			success: function(){

...
if($display_status) {
	...
	if(isset($_POST['action']) && ($_POST['action'] == "active" || $_POST['action'] == "inactive") && isset($_POST['post']) && !empty($_POST['post']))
	{
		...
		$obj_template_volume->updateStatus($status,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	
	}else if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post'])){
	 //printr($_POST['post']);die;
		$obj_template_volume->updateStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
                 ...
                 <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New Template Volume </a>
                      ...
                      <div class="row">
                          <div class="col-lg-4">
                              <div class="form-group">
                                <label class="col-lg-5 control-label">Template Volume</label>
                                <div class="col-lg-7">
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php	
                  $total_unit = $obj_template_volume->getTotalTemplateVolume($filter_data);
                  $pagination_data = '';
                  if($total_unit){
                        if (isset($_GET['page'])) {
                            $page = (int)$_GET['page'];
                      ...
                      $products = $obj_template_volume->getTemplateVolumes($option,$filter_data);
					 // printr($products);die;
					  foreach($products as $product){ 
                        ...
                        <tr <?php echo ($product['status']==0) ? 'style="background-color:#FADADF" ' : '' ; ?>>
                          ...
                          <td><?php $volume = $product['volume'].' '.$product['measurement'];  
					         echo $volume;?></td>
                          <td>
                          ...
                          <td>	
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&product_id='.$product['product_id'],'',1); ?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                           </td>
                        </tr>
                        <?php
                      }
...
<script type="application/javascript">
	$('input[name=status]').change(function() {
		...
		//alert(status_value);
		var status_url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&ajax=UpdateTemplateVolumeStatus', '',1);?>");
        //alert(status_url);
		$.ajax({			
			url : status_url,
			type :'post',

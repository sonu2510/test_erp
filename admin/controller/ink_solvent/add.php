...
if(isset($_GET['solvent_id']) && !empty($_GET['solvent_id'])){
	if(!$obj_general->hasPermission('edit',$menuId)){
		...
		$solvent = $obj_ink->getInkSolvent($solvent_id);
		$edit = 1;
	}
...
if($display_status){
	...
	if(isset($_POST['btn_save'])){
		$post = post($_POST);		
		$insert_id = $obj_ink->addInkSolvent($post);
	...
	if(isset($_POST['btn_update']) && $edit){
		$post = post($_POST);
		//printr($post);die;
		$solvent_id = $solvent['ink_solvent_id'];
		$obj_ink->updateInkSolvent($solvent_id,$post);
        ...
        <section class="panel">
          ...
          <div class="panel-body">
            ...
            <div class="form-group">
                ...
                <div class="col-lg-4">
                  	<input type="text" name="unit" id="unit" value="<?php echo isset($solvent['ink_solvent_unit'])?$solvent['ink_solvent_unit']:'';?>" class="form-control validate[required]">
                </div>
              </div>
               <!--ruchi-->
               <div class="form-group">
                <label class="col-lg-3 control-label">Minimum Product Quintity</label>
                <div class="col-lg-4">
                  	<input type="text" name="min_qty" id="min_qty" value="<?php echo isset($solvent['ink_solvent_min_qty'])?$solvent['ink_solvent_min_qty']:'';?>" class="form-control validate[required,custom[number]]">
                </div>
              </div> 

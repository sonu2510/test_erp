...
if($display_status){
	$data = $obj_adhesive->viewAdhesive_report($adhesive_id);
...
$("#excel_link").click(function(){
	var adhesive_id = <?php echo $adhesive_id?>;
	//alert(adhesive_id);
	
	var url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&fun=viewAdhesive_report', '',1);?>");
	
	 $.ajax({
        url: url, // the url of the php file that will generate the excel file
       	data : {adhesive_id : adhesive_id},
		method : 'post',
        success: function(response){
			excelData = 'data:application/excel;charset=utf-8,' + encodeURIComponent(response);
			 $('<a></a>').attr({
							'id':'downloadFile',
							'download': 'Adhesive-report.xls',
							'href': excelData,
							'target': '_blank'
					}).appendTo('body');

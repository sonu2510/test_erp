...
if(isset($_GET['indent_id']) && !empty($_GET['indent_id'])){
	...
	if(!$obj_general->hasPermission('view',$menuId)){
		...
		$cond='AND ph.approve_qty!=0 GROUP BY purchase_indent_items_id';
    ...
    <div class="row">
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
		  	<span>Approve Indent Listing</span>
           ...
           <?php
							...
							?>
								<a class="btn btn-info btn-xs" data-trigger="hover" data-toggle="popover" data-html="true" data-placement="top" data-content='<?php echo $addedByInfo;?>' title="" data-original-title="<b><?php echo $addedByName;?></b>"><?php echo $addedByData['user_name'];?></a>
                                 ...
                                 <strong><?php echo dateFormat(4,$inventory[0]['reminder_date']);?></strong>    <i class="fa fa-clock-o"></i>  
                                 <br>
                                 </p> 
                                 <div class="line"></div>
                           <table class="table"> 
                         ...
                         <td><?php echo $items['app_qty']; $total=$total+$items['app_qty'];?>&nbsp;&nbsp;
						 <?php echo $product_unit;?>
                              </td>
                              <?php $pen=$items['purchase_indent_items_id'];
							 $cond='AND approve_qty!=0 ';
				...
				$username.='<span style="color:#26B756">'.$process_by['user_name'].'</span> On <span>'.dateFormat(4,$inventory[0]['date']).'</span>    <span style="color:#00F">'.$cancle['approve_qty'].'</span>&nbsp;'.$product_unit.'  </div><br>';
							}?>
                             
                            <a class="btn btn-info btn-xs" data-trigger="hover" data-toggle="popover" data-html="true" data-placement="top" data-content=
                             '<?php echo $username;?>' title="" data-original-title="Approve">View History</a>
                              </td>
                              </tr>
							  <?php
                              ...
                              <tr> <td colspan="3" class="text-right no-border"><strong>Total</strong></td>
                                ...
                                <p class="m-t m-b">Discription: 
                                 ...
                                 <a class="btn btn-default" href="<?php echo $obj_general->link($rout,'mod=index&status='."4", '',1);?>" style="float: right;">Cancel</a>
                                 </p> 

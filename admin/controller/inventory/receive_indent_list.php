    ...
    <div class="row">
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
		  	<span>Receive Indent Listing</span>
           ...
           <?php
							...
							?>
								<a class="btn btn-info btn-xs" data-trigger="hover" data-toggle="popover" data-html="true" data-placement="top" data-content='<?php echo $addedByInfo;?>' title="" data-original-title="<b><?php echo $addedByName;?></b>"><?php echo $addedByData['user_name'];?></a>
                                 ...
                                 <strong><?php echo dateFormat(4,$inventory[0]['due_date']);?></strong></strong>    <i class="fa fa-clock-o"></i>  
                                 <br>
                                 </p> 
                                 <div class="line"></div>
                           <table class="table"> 
                           ...
                           <tr>
                            <th>S.NO</th>
                             <th>ITEMS</th> 
							   ...
							   if($items['table_name'] == 'product_accessorie')
							  {
								...
								$inv = $obj_inventory->getName($cond,$items['table_name']);
									...
									$prodcut_unit = $inv['product_accessorie_unit'];
							  }
							   if($items['table_name'] == 'product_material')
							  {
								...
								$inv = $obj_inventory->getName($cond,$items['table_name']);
									...
									$prodcut_unit = $inv['material_unit'];
							  }
                             ...
                             <?php
							  ...
							  $cond='group by purchase_indent_items_id order by rec_qty ASC';
								 $pending = $obj_inventory->getRecive($pen,$cond); 
							//printr($pending);
								  if($pending[0]['max_qty']==0)
								  {
									 $recq=0;
									  $k='';
								  }
								  else
								  {	$recq=$pending[0]['max_qty'];
									  $k=$prodcut_unit;
								  }
							  ?>
                             <td><?php echo $recq; $total=$total+$pending[0]['max_qty'];?>&nbsp;&nbsp;<?php echo $k;?></td><br />

                             <?php $pen=$items['purchase_indent_items_id'];
							 $cond='AND rec_qty!=0 AND rec=2';
				...
				$username.='<div><span style="color:#26B756">'.$process_by['user_name'].'</span> On <span>'.dateFormat(4,$inventory[0]['date']).'</span>    <span style="color:#00F">'.$cancle['rec_qty'].'</span> &nbsp;'.$prodcut_unit.'</div>';
				
							}?>
                              <a class="btn btn-info btn-xs" data-trigger="hover" data-toggle="popover" data-html="true" data-placement="top" data-content=
                             '<?php echo $username;?>' title="" data-original-title="Receive">View History</a>
                              </td>
                              </tr>
							  <?php
                              ...
                              <tr> <td colspan="3" class="text-right no-border"><strong>Total</strong></td>
                                ...
                                <p class="m-t m-b">Discription: 
                                 ...
                                 <br> Reminder Date: 
                                 <span class="label bg-success"><?php echo dateFormat(4,$inventory[0]['reminder_date']);?></strong></span>
                                 <a class="btn btn-default" href="<?php echo $obj_general->link($rout,'mod=index&status='."1", '',1);?>" style="float: right;">Cancel</a>

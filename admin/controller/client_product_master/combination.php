...
if($display_status) {
	...
	{	if(!$obj_general->hasPermission('edit',$menuId)){
			...
			page_redirect($obj_general->link($rout, '&mod=combination&product_type_id='.$_GET['product_type_id'], '',1));
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> <span><?php echo $display_name;?> Listing</span>
          	<span class="text-muted m-l-small pull-right">         			
               <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add&product_type_id='.$_GET['product_type_id'], '',1);?>" ><i class="fa fa-plus"></i> New Product </a>
          ...
          <div class="panel-body">
                ...
                <form class="form-horizontal" method="post" data-validate="parsley" action="<?php echo $obj_general->link($rout, '&mod=combination&product_type_id='.$_GET['product_type_id'], '',1); ?>">
                  ...
                  <footer class="panel-footer <?php echo $class; ?>">
                    <div class="row">
                       <div class="col-lg-12">
                        ...
                        <a href="<?php echo $obj_general->link($rout, '&mod=combination&product_type_id='.$_GET['product_type_id'], '',1); ?>" class="btn btn-info btn-sm pull-right" ><i class="fa fa-refresh"></i> Refresh</a>
                       </div> 
                    </div>
                  </footer>                                  
              </section>
          ...
          <div class="row">
             <div class="col-lg-3 pull-right">	
                 ...
                 <option value="<?php echo $obj_general->link($rout, '&mod=combination&product_type_id='.$_GET['product_type_id'], '',1);?>" selected="selected">--Select--</option>
                    	<?php 
						...
						?>
                        		...
                        		<option value="<?php echo $obj_general->link($rout, '&mod=combination&product_type_id='.$_GET['product_type_id'].'limit='.$display_limit, '',1);?>" selected="selected"><?php echo $display_limit; ?></option>				
						<?php } else { ?>
                            	<option value="<?php echo $obj_general->link($rout,  '&mod=combination&product_type_id='.$_GET['product_type_id'].'limit='.$display_limit, '',1);?>"><?php echo $display_limit; ?></option>
                        <?php } ?>
                        <?php } ?>
                 </select>
             </div>
          	...
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  ...
                  <tbody>
					...
					$count = $obj_product->getcount($filter_data,$product_type_id);
					$pagination_data = '';
					if($count){
							if (isset($_GET['page'])) {
								$page = (int)$_GET['page'];
						...
						$results = $obj_product->getvalue($option,$filter_data,$product_type_id);	
						...
						<tr <?php echo ($result['status']==0) ? 'style=background-color:#FADADF' : '' ; ?>>
						  ...
						  <td>
                          	<a href="<?php echo $obj_general->link($rout,'mod=add&client_product_id='.encode($result['client_product_id']).'&product_type_id='.encode($product_type_id),'',1)?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
						   </td>
						</tr>
						<?php
						}
                        ...
                        $pagination->url = $obj_general->link($rout, '&mod=combination&product_type_id='.$_GET['product_type_id'].'&page={page}', '',1);//HTTP_ADMIN.'index.php?rout='.$rout.'&page={page}';
                        $pagination_data = $pagination->render();
                        //echo $pagination_data;die;
					}else{
						echo "<tr><td colspan='5'>No Combination found !</td></tr>";
					} ?>
                  </tbody>
                </table>

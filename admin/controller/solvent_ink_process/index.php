...
{	if(!$obj_general->hasPermission('edit',$menuId)){
		...
		$obj_sol_ink_process->updateStatus($status,$_POST['post']);
...
else if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post']))
{
	if(!$obj_general->hasPermission('delete',$menuId)){
		...
		$obj_sol_ink_process->updateStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
}
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
                 ...
                 <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>">&nbsp;&nbsp;&nbsp;<i class="fa fa-plus">&nbsp;&nbsp;&nbsp;</i>New Solvent Ink Process </a>
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php	
		  $total_job = $obj_sol_ink_process->getTotalSolventInkProcess($filter_data);
...
//                      printr($limit);
		   $solvent_ink_process = $obj_sol_ink_process->getSolventInkProcess($option,$filter_data);
	
		  // printr($solvent_ink_process);
			  foreach($solvent_ink_process as $sol_ink){ 
			  $camist_name =$obj_sol_ink_process->getchemist_id($sol_ink['chemist_id']);
		//	  printr($sol_ink);
                        ?>
                        <tr <?php echo ($sol_ink['status']==0) ? 'style="background-color:#FADADF" ' : '' ; ?>>
                          <td><input type="checkbox" name="post[]" value="<?php echo $sol_ink['solvent_ink_p_id'];?>"></td>
                          <td><?php  echo dateformat(4, $sol_ink['date']);  ?></td>
                          <td><?php  echo $sol_ink['job_no'];  ?></td>
                          <td><?php  echo $sol_ink['job_name'];  ?></td>
                          <td><?php  echo ($sol_ink['shift']==1)?'Day':'Night';  ?></td>
                          <td><?php  echo $sol_ink['operator_name']  ?></td>
                          <td><?php  echo $camist_name ?></td>
                          <td>
                              
                                <div data-toggle="buttons" class="btn-group">
                                    <label class="btn btn-xs btn-success <?php  echo ($sol_ink['status']==1) ? 'active' : '';?> "> <input type="radio" 
                                     name="status" value="1" id="<?php  echo $sol_ink['solvent_ink_p_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label>                                   
                                    <label class="btn btn-xs btn-danger <?php  echo ($sol_ink['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                        name="status" value="0" id="<?php  echo $sol_ink['solvent_ink_p_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label> 
                                </div>
                              
						  </td>
                          <td>	
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&solvent_ink_process='.encode($sol_ink['solvent_ink_p_id']),'',1); ?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>

...
if(isset($_GET['job_id']) && !empty($_GET['job_id'])){
	...
	//$job_detail = $obj_printing_job->viewjob_details($job_id);
//	printr($job_id);	
	$job_detail = $obj_job->viewjob_details($job_id);
}
        ...
        <div class="col-sm-12">
            <section class="panel">  
                ...
                <header class="panel-heading bg-white">
                 <span><?php echo $display_name;?> Job Detail</span>
...
//	alert(job_id);
	...
	var url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&fun=displaydata', '',1);?>");
	 ...
	 $.ajax({
        ...
        success: function(response){
			excelData = 'data:application/excel;charset=utf-8,' + encodeURIComponent(response);
			 $('<a></a>').attr({
							'id':'downloadFile',
							'download': 'PrintingJob-report.xls',
							'href': excelData,
							'target': '_blank'
					}).appendTo('body');
...
function test() {
	 ...
	 var html="<html>";
	     ...
	     html+='<style>.col-lg-3 {width: 15%;}#client {   float: left;   padding-left: 6px;}  h1 {	background:#333;    border-bottom: 1px solid #5d6975;    border-top: 1px solid #5d6975;    color: #FFF;    font-size:  12px;    font-weight: normal;    line-height: 1.4em;    margin: 0 0 20px;    text-align: center;}article, article address, table.meta, table.inventory { margin: 0 0 3em; }table.meta, table.balance { float: right; width: 50%; }table.meta:after, table.balance:after { clear: both; content: ""; display: table; }table.meta th { width: 40%;  font-size:  18px; }table.meta td { width: 60%;   font-size:  18px; }table { font-size:  18px; table-layout: fixed; width: 100%; }table { border-collapse: separate; border-spacing: 1px; font-size:  18px;  }th, td { border-width: 1px;position: relative; text-align: left;  font-size:  18px; }th, td { border-radius: 0em; border-style: solid; font-size:  18px;}th { background: #EEE; border-color: #BBB; font-size:  18px;td { border-color: #DDD; font-size:  font-size:  18px;}.sign_td {	height:100px;}div #express{	margin:30px 40px;}</style>';

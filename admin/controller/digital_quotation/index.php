	...
	if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post'])){
		if(!$obj_general->hasPermission('delete',$menuId)){
			...
			//	printr($_POST['post']);die;
				$obj_digital->deleteQuotation($quotation_id);
			}
                          ...
                          <div class="col-lg-4">
                           ...
                           <div class="row">
                                <div class="col-lg-4">
                                    <div class="form-group">
                                      ...
                                      <?php	 	$products = $obj_digital->getProductList();	?>
                                      <div class="col-lg-7">
                           ...
                           <div class="col-lg-4">
                              <div class="form-group">
                                ...
                                <?php							
								                	$userlist = $obj_digital->getUserList();
							                   	?>
                                <div class="col-lg-7">
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table table-striped b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php $Total_quo = $obj_digital->getQuotationData($obj_session->data['ADMIN_LOGIN_SWISS'],$obj_session->data['LOGIN_USER_TYPE'],$cond,$filter_data,$option);
                  ...
                  if($total_quo){
                      ...
                      $option = array(
                            'sort'  => 'dqi.digital_quotation_id',
                            'order' => 'DESC',
                            'start' => ($page - 1) * LISTING_LIMIT,
                            'limit' => LISTING_LIMIT
                      );	
                    $data_quo = $obj_digital->getQuotationData($obj_session->data['ADMIN_LOGIN_SWISS'],$obj_session->data['LOGIN_USER_TYPE'],$cond,$filter_data,$option);
                    foreach($data_quo as $quo){
                            $postedByData = $obj_digital->getUser($quo['user_id'],$quo['user_type_id']);
                        ?>
                        ...
                        <tr  <?php echo ($quo['quotation_status']==0) ? 'style="background-color:#f2dede" ' : '' ; ?>> 
                            ...
                            <td><a href="<?php echo $obj_general->link($rout, '&mod=view&quotation_id='.encode($quo['digital_quotation_id']).'&filter_edit='.$filter_edit, '',1);?>"><?php echo $i;?></a></td>
                        	<td><a href="<?php echo $obj_general->link($rout, '&mod=view&quotation_id='.encode($quo['digital_quotation_id']).'&filter_edit='.$filter_edit, '',1);?>"><?php echo $quo['digital_quotation_no'];?><br /><small class="text-muted"><?php echo dateFormat(4,$quo['date_added']);?></small></a></td>

                        	<td><a href="<?php echo $obj_general->link($rout, '&mod=view&quotation_id='.encode($quo['digital_quotation_id']).'&filter_edit='.$filter_edit, '',1);?>"><?php echo $quo['client_name'];?><br /><small class="text-muted"><?php echo $quo['email'];?></small></a></td>

                          <td><a href="<?php echo $obj_general->link($rout, '&mod=view&quotation_id='.encode($quo['digital_quotation_id']).'&filter_edit='.$filter_edit, '',1);?>"><?php echo $quo['country_name'];?></a></td>
                          	<td><a href="<?php echo $obj_general->link($rout, '&mod=view&quotation_id='.encode($quo['digital_quotation_id']).'&filter_edit='.$filter_edit, '',1);?>"><?php echo $quo['name'];?></a></td>
                          <td>

...
if($display_status){
	...
	if(isset($_POST['btn_save'])){
		...
		if(!empty($addedByInfo['lang_id']) && (( $_SESSION['LOGIN_USER_TYPE']=='2' && $_SESSION['ADMIN_LOGIN_SWISS']=='258') OR ($_SESSION['LOGIN_USER_TYPE']=='1')) )
		{
		    $obj_quotation->sendQuotationEmailInOtherLang($quotation_id);
		    //die;
		}
	...
	if(isset($_POST['btn_sendemail'])){
		if(isset($_POST['smail']) && !empty($_POST['smail'])){
			...
			if(!empty($addedByInfo['lang_id']) && (( $_SESSION['LOGIN_USER_TYPE']=='2' && $_SESSION['ADMIN_LOGIN_SWISS']=='258') OR ($_SESSION['LOGIN_USER_TYPE']=='1')) )
			{
			    $obj_quotation->sendQuotationEmailInOtherLang($quotation_id,$gemail,$setCurrencyId,$_POST['sel_currency_sec'],$_POST['currency_rate']);
			    //die;
			}
			...
			if($_SESSION['ADMIN_LOGIN_SWISS']=='37' && $_SESSION['LOGIN_USER_TYPE']=='2')
		        $obj_quotation->sendQuotationEmailInItalian($quotation_id,$gemail,$setCurrencyId,$_POST['sel_currency_sec'],$_POST['currency_rate']);
			elseif($_SESSION['ADMIN_LOGIN_SWISS']=='199' && $_SESSION['LOGIN_USER_TYPE']=='2')
		        $obj_quotation->sendQuotationEmailInSpanishUK($quotation_id,$gemail,$setCurrencyId,$_POST['sel_currency_sec'],$_POST['currency_rate']);    
		    else
...
//	if($_SESSION['ADMIN_LOGIN_USER_TYPE']=='1')
	//{
    	//printr($data);
	//}
?>
            ...
            <section class="panel">  
                <header class="panel-heading bg-white">
                 ...
                 <span class="text-muted m-l-small pull-right">
                    ...
                    <?php }  ?>
                    <a class="label bg-primary pdfclsmail" href="javascript:void(0);"><i class="fa fa-envelope"></i> Mail Format PDF</a>
                     <a class="label bg-success" href="javascript:void(0);" onclick="wordlink('<?php echo rawurlencode($quotation_id);?>','<?php echo rawurlencode($data[0]['multi_quotation_number']);?>')"><i class="fa fa-print"></i>Mail Format Doc</a>
                 </span>
              </header>
                 ...
                 <form class="form-horizontal" method="post" name="form" id="form" enctype="multipart/form-data">
					  ...
					  if($allow_currency_status && $data[0]['status'] == 1){
						 ...
						 // $data[0]['currency']:'INR'
									      ...
									      <div class="col-lg-3">
                                        	<select name="sel_currency" id="sel_currency" class="form-control" >
                                                <?php foreach($currencys as $crr)
    									            {?>
                                                        <option value="<?php echo $crr['currency_code'];?>" price = "<?php echo $crr['price'];?>"  <?php if($data[0]['currency']==$crr['currency_code']) echo 'selected' ;?> attr_curr="drop"><?php echo $crr['currency_code'];?></option>
                                              <?php } ?>
                                              </select>
                                        
                                        
                                        	<select name="sel_currency_secondary" id="sel_currency_secondary" class="form-control"   style="display:none">
                                                <option value="<?php echo $data[0]['currency'];?>" ><?php echo $data[0]['currency'];?></option>
                                                <?php foreach($currencys as $crr)
									            {?>
								...
								<?php }
                                	...
                                	<div class="col-lg-4">
                                        <?php 
											...
											{
										?>
                                                <input type="text" name="sel_currency" id="sel_currency" value="<?php echo $data[0]['currency'];?>" attr_curr="select" class="form-control" readonly="readonly"/>
						  ...
						  { ?>
						      <input type="hidden" name="sel_currency" id="sel_currency" value="<?php echo $data[0]['currency'];?>" attr_curr="select" class="form-control" readonly="readonly"/>
					<?php }
					  ?>
                      <div class="form-group">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-lg-4">
						   ...
						   <?php echo $add_url_to;?>
                            <input type="hidden" name="customer_email" id="customer_email" value="<?php if($addedByInfo['user_id']=='41'){ echo 'swisspac@lmfe.com.ph'; } else echo $addedByInfo['email'];?>" />
                            </label>
                        </div>
                      </div>
                      <div class="form-group">
												...
												?>
                                                            ...
                                                            <th><?php echo $skey;?></th>
                                                             <td>
															   ...
															   //echo '<br>'.$soption['slider_place'].'<br>';
															   
															   //if($soption['slider_place'] == 'Slider Zipper On Top')
															        //echo $soption['tamperevident'].'<br>'; 
															   ...
															   if($soption['spout_txt'] != 'No Spout')
															   {
																...
																else echo ' (Custom)'.' ('.$soption['make'].')'; ?>
															 <?php
															 //printr($soption['quantity_option']);
															 
																if($data[0]['product_id']==6){
																 foreach($soption['quantity_option'] as $squantityKey=>$squantity)
                                                                     { 
                                                                        if($squantityKey=='Total Kgs' || $squantityKey=='Total Mtr' || $squantityKey=='Total Pieces'|| $squantityKey=='Total Pieces Per Kg' ){
...
<script>
    ...
    jQuery(document).ready(function(){
		...
		$(".sendmailcls").click(function(){	
		    ...
		    if(attr_curr=='drop')
		        var selcurrency = '<?php echo $data[0]['currency']; ?>';
		    else      
		...
		$(".pdfcls").click(function(){			
			...
			<?php if($_SESSION['ADMIN_LOGIN_SWISS']=='1' && $_SESSION['LOGIN_USER_TYPE']=='1') {?>
			      var url = '<?php echo HTTP_SERVER.'pdf/pdf.php?mod='.encode('productQuotation_new').'&token='.rawurlencode(encode($quotation_id)).'&ext='.md5('php');?>&ssc='+selcurrency;//+'&sscr='+btoa(rate)
...
$('#sel_currency ,#sel_currency_secondary').change(function(){
    var value = $(this).val();
    
    var old = '<?php echo $data[0]['currency']; ?>';
...
//alert(value);alert(price);
    ...
    else
    {
		...
		$("#sel_currency_secondary option[value='"+value+"']").attr("selected","selected");
		$("#sel_currency_rate").attr('readonly','readonly');
...
$(".pdfclsmail").click(function(){
   
   var customer_email=$("#customer_email").val();
   var url = '<?php echo HTTP_SERVER.'pdf/pdf.php?mod='.encode('quotationsendmail').'&token='.rawurlencode(encode($quotation_id)).'&ext='.md5('php');?>&customer_email='+customer_email;//+'&sscr='+btoa(rate)
   window.open(url, '_blank');
   
});
function wordlink(id,no){
    var url = '<?php echo HTTP_SERVER.'word/multiQuoMailValue.php?mod='.encode('multiQuoMailValue').'&ext='.md5('php');?>&token='+id+'&quo_no='+no;
    window.open(url, '_blank');
    return false;
}
</script>
...
$(".pdfclsmail").click(function(){
   ...
   var customer_email=$("#customer_email").val();
   var url = getUrl("<?php //echo $obj_general->ajaxLink($rout, 'mod=ajax&fun=sendmailPDFFormat', '',1);?>");
   $.ajax({
				url : url,
				method : 'post',
				data : {quotation_id:quotation_id,customer_email:customer_email},
				success: function(response){

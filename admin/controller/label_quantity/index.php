          ...
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
				...
				<?php if($obj_general->hasPermission('add',$menuId)){ ?>
   					<a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New Label Qty </a>
            ...
            <table class="table table-striped b-t text-small table-hover">
			  ...
			  if($total_layer){
				  ...
				  foreach($quantitys as $quantity){ 
					?>
                    <tr>
                      <td><input type="checkbox" name="post[]" value="<?php echo $quantity['label_quantity_id'];?>"></td>
                      ...
                      <td>
                      		<a href="<?php echo $obj_general->link($rout, 'mod=add&label_quantity_id='.encode($quantity['label_quantity_id']), '',1); ;?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                       </td>
                    </tr>
                    <?php
				  }

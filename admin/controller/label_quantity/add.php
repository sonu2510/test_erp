...
//Start : edit
$edit = '';
if(isset($_GET['label_quantity_id']) && !empty($_GET['label_quantity_id'])){
	if(!$obj_general->hasPermission('edit',$menuId)){
		$display_status = false;
	}else{
		$quantity_id = base64_decode($_GET['label_quantity_id']);
...
if($display_status){
	...
	if(isset($_POST['btn_save'])){
		$post = post($_POST);		
		$insert_id = $obj_quantity->addQuantity($post);
	...
	if(isset($_POST['btn_update']) && $edit){
		$post = post($_POST);
		$quantity_id = $editData['label_quantity_id'];
          ...
          <div class="panel-body">
            <form class="form-horizontal" method="post" name="frm_add" id="frm_add" enctype="multipart/form-data">
              ...
              <div class="form-group">
                ...
                <div class="col-lg-8">
                  <input type="text" name="pm_quantity" value="<?php echo isset($editData['plus_minus_quantity'])?$editData['plus_minus_quantity']:'';?>" class="form-control validate[required]">
                </div>
              </div>

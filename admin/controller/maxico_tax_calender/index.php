				  ...
				  if($tax_total){
					  ...
					  foreach($taxes as $tax){ 
                        ...
                        <tr <?php echo ($tax['status']==0) ? 'style=background-color:#FADADF' : '' ; ?> >
                          <td><input type="checkbox" name="post[]" value="<?php echo $tax['tax_calender_id'];?>"></td>
                          ...
                          <div data-toggle="buttons" class="btn-group">
                                <label class="btn btn-xs btn-success <?php echo ($tax['status']==1) ? 'active' : '';?> "> <input type="radio" 
                                 name="status" value="1" id="<?php echo $tax['tax_calender_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label>                                   
                                <label class="btn btn-xs btn-danger <?php echo ($tax['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                    name="status" value="0" id="<?php echo $tax['tax_calender_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label> 
                           </div>
                           </td>
                          <td>
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&tax_calender_id='.encode($tax['tax_calender_id']), '',1); ;?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                          </td>
                            <?php
                           if($_SESSION['ADMIN_LOGIN_SWISS']=='1' && $_SESSION['LOGIN_USER_TYPE']=='1')
                           {
						    ?>
                           <td> <input type="button" name="send_mail<?php echo $i;?>" id="send_mail<?php echo $i;?>" value="Send Mail"  class="btn btn-success" onclick="acceptmail(<?php echo $i;?>,<?php echo $tax['tax_calender_id'];?>)" /></td>
...
<script type="application/javascript">
	   ...
	   $('input[type=radio][name=status]').change(function() {
		...
		var status_url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&fun=updateMaxicotax', '',1);?>");
        $.ajax({			
			url : status_url,
			type :'post',
...
function acceptmail(id,tax_calender_id)
{	
	//alert(tax_calender_id);
	$(".note-error").remove();
	...
	$.ajax({
		...
		method : 'post',
		data : {tax_calender_id : tax_calender_id,adminEmail:adminEmail},

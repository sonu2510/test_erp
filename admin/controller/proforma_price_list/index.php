           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table table-striped b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php
				    $total_branch = $obj_pricelist->getTotalBranch($filter_data);
					$pagination_data = '';
                  if($total_branch){
                        if (isset($_GET['page'])) {
                            $page = (int)$_GET['page'];
				  ...
				  $option = array(
                      ...
                      $products = $obj_pricelist->getBranchs($option);
					 ...
					 //printr($products);die;
                        ...
                        <tr>
							  ...
							  <td><?php echo $branch['email'];?></td>
							   <td><?php echo $obj_pricelist->getCountryName($branch['country_id']);?></td>
							  <td><?php echo $obj_pricelist->getdefaultcurrencyCode($branch['default_curr']);?></td>
							  ...
							  <td>
							        <a href="<?php echo $obj_general->link($rout, 'mod=view&user_id='.encode($branch['user_id']).'&user_type_id='.encode($branch['user_type_id']), '',1); ;?>"  name="btn_edit" class="btn btn-info btn-xs">Edit </a>
							        <a href="<?php echo $obj_general->link($rout, 'mod=viewReport&user_id='.encode($branch['user_id']).'&user_type_id='.encode($branch['user_type_id']), '',1); ;?>"  name="" class="label bg-primary">Price Sheet</a>
							  </td>
                    </tr>
                        <?php
                      }

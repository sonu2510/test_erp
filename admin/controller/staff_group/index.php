...
if(isset($_POST['action']) && ($_POST['action'] == "active" || $_POST['action'] == "inactive") && isset($_POST['post']) && !empty($_POST['post']))
{
	if(!$obj_general->hasPermission('edit',$menuId)){
		...
		$obj_department->updateStatus($status,$_POST['post']);
	...
	if(!$obj_general->hasPermission('delete',$menuId)){
		...
		$obj_department->updateStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
}
          ...
          <form name="form_list" id="form_list" method="post">
           <input type="hidden" id="action" name="action" value="" />
            ...
            <table class="table table-striped b-t text-small">
              ...
              <tbody>
              <?php
              $total_department = $obj_department->getTotalStaffGroup();
			  ...
			  if($total_department){
				  ...
				  $StaffGroup = $obj_department->getStaffGroup($option);
				  foreach($StaffGroup as $StaffGroup){ 
					?>
                    <tr>
                      <td><input type="checkbox" name="post[]" value="<?php echo $StaffGroup['staff_group_id'];?>"></td>
                      <td><?php echo $StaffGroup['staff_group_name'];?></td>
                      <td><label class="label   
                        <?php echo ($StaffGroup['status']==1)?'label-success':'label-warning';?>">
                        <?php echo ($StaffGroup['status']==1)?'Active':'Inactive';?>
                        </label>
                      </td>
                      <td>
                      		<a href="<?php echo $obj_general->link($rout, 'mod=add&staffgroup_id='.encode($StaffGroup['staff_group_id']), '',1); ;?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                       </td>
                    </tr>
                    <?php
				  }

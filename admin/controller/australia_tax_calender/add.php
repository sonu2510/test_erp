...
if(isset($_GET['tax_aus_calender_id']) && !empty($_GET['tax_aus_calender_id'])){
	if(!$obj_general->hasPermission('edit',$menuId)){
		$display_status = false;
	}else{
		$tax_aus_calender_id = base64_decode($_GET['tax_aus_calender_id']);
		$tax_record = $obj_tax_calender->getTaxRecord($tax_aus_calender_id);
		$tax_record_img = $obj_tax_calender->getTaxImgRecord($tax_aus_calender_id);
...
if($display_status){
	...
	if(isset($_POST['btn_update']) && $edit){
		...
		$tax_aus_calender_id = $tax_record['tax_aus_calender_id'];
		$obj_tax_calender->updateTax($tax_aus_calender_id,$post);
                                        ...
                                        <table class="table table-striped b-t text-small ">
                                             ...
                                             <tbody class="tbody">
                                                <?php $i=1;
                                                  ...
                                                  <input type="hidden" name="aus_image_id[<?php echo $i; ?>]" value="<?php echo $img_result['aus_image_id'];?>" />
                                                     <!-- <td> <?php //echo $i; ?> </td>-->
                                                      <td> <?php echo dateFormat(4,$img_result['date_added']); ?> </td>
                                                      <td><a target="_blank" href="<?php echo HTTP_UPLOAD.'admin/tax_img/100_'.$img_result['image_name']; ?>" ><?php echo $img_result['image_name']; ?> </a></td>
                                                      <td> <?php echo $user_name['user_name']; ?><input type="hidden" name="img_nm[<?php echo $i; ?>]" value="<?php ?>" /></td>
                                                      <td> <a class="label bg-danger remove" style="margin-left:4px;" onclick="remove_record(<?php echo $img_result['aus_image_id'];?>,<?php echo $i; ?>)"><i class="fa fa-trash-o"></i> Remove</a></td> 
                 ...
                 <div class="form-group">
                            ...
                            <div class="col-lg-4">
                              <div data-toggle="buttons" class="btn-group m-t-n-mini m-r-n-mini"> 
                                ...
                                <label class="btn btn-sm btn-white <?php echo ($reminder_status=='1') ? 'btn-on active' : '';?>"><input type="radio" id="option1" name="reminder" value="1"> ON </label> 
                                <label class="btn btn-sm btn-white <?php echo ($reminder_status=='0') ? 'btn-on active' : '';?>"><input type="radio" id="option2" name="reminder" value="0"> OFF </label> 
                              </div>
                            </div>
                         </div>

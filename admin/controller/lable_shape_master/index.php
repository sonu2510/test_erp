        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
            ...
            <span class="text-muted m-l-small pull-right">
                 ...
                 <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New Shape </a>
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table table-striped b-t text-small table-hover">
                  <thead>
                    <tr>
                      ...
                      <th class="th-sortable <?php echo ($sort_order=='ASC') ? 'active' : ''; ?> ">
                          Shape Name
                          <span class="th-sort">
                            <a href="<?php echo $obj_general->link($rout, 'sort=shape_name'.'&order=ASC', '',1);?>">
                            <i class="fa fa-sort-down text"></i>
                            <a href="<?php echo $obj_general->link($rout, 'sort=shape_name'.'&order=DESC', '',1);?>">
                  ...
                  <tbody>
                  <?php
                  $total_shape = $obj_printing->getTotalShape();
                  $pagination_data = '';
                  if($total_shape){
                        if (isset($_GET['page'])) {
                            $page = (int)$_GET['page'];
                      ...
                      $Shapes = $obj_printing->getShape($option);
                      foreach($Shapes as $shape){ 
                        ?>
                        <tr>
                          <td><input type="checkbox" name="post[]" value="<?php echo $shape['shape_master_id'];?>"></td>
                          <td><?php echo ucwords($shape['shape_name']);?></td>
                        
                          <td>
                            <label class="label <?php echo ($shape['status']==1)?'label-success':'label-warning';?>">
                                <?php echo ($shape['status']==1)?'Active':'Inactive';?>
                            </label>
                          </td> 
                          <td>
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&shape_master_id='.encode($shape['shape_master_id']), '',1); ;?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>

...
{	if(!$obj_general->hasPermission('edit',$menuId)){
		...
		$obj_mix_sol_process->updateStatus($status,$_POST['post']);
...
else if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post']))
{
	if(!$obj_general->hasPermission('delete',$menuId)){
		...
		$obj_mix_sol_process->updateStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
}
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
                 ...
                 <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>">&nbsp;&nbsp;&nbsp;<i class="fa fa-plus">&nbsp;&nbsp;&nbsp;</i>New Mix Solvent Process </a>
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php	
		  $total_job = $obj_mix_sol_process->getTotalMixSolventProcess($filter_data);
...
//                      printr($limit);
		   $mix_solvent_process = $obj_mix_sol_process->getMixSolvenProcess($option,$filter_data);
		  // printr($solvent_ink_process);
			  foreach($mix_solvent_process as $mix_sol){ 
			       $camist_name =$obj_mix_sol_process->getchemist_id($mix_sol['chemist_id']);
		//	  printr($mix_sol);
                        ?>
                        <tr <?php echo ($mix_sol['status']==0) ? 'style="background-color:#FADADF" ' : '' ; ?>>
                          <td><input type="checkbox" name="post[]" value="<?php echo $mix_sol['mix_solvent_p_id'];?>"></td>
                          <td><?php  echo dateformat(4, $mix_sol['date']);  ?></td>
                          <td><?php  echo $mix_sol['job_no'];  ?></td>
                          <td><?php  echo $mix_sol['job_name'];  ?></td>
                          <td><?php  echo ($mix_sol['shift']==1)?'Day':'Night';  ?></td>
                          <td><?php  echo $mix_sol['operator_name']  ?></td>
                          <td><?php  echo  $camist_name; ?></td>
                          <td>
                              
                                <div data-toggle="buttons" class="btn-group">
                                    <label class="btn btn-xs btn-success <?php  echo ($mix_sol['status']==1) ? 'active' : '';?> "> <input type="radio" 
                                     name="status" value="1" id="<?php  echo $mix_sol['mix_solvent_p_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label>                                   
                                    <label class="btn btn-xs btn-danger <?php  echo ($mix_sol['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                        name="status" value="0" id="<?php  echo $mix_sol['mix_solvent_p_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label> 
                                </div>
                              
						  </td>
                          <td>	
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&mix_solvent_process='.encode($mix_sol['mix_solvent_p_id']),'',1); ?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
...
<script type="application/javascript">
	...
	$('input[type=radio][name=status]').change(function() {
		...
		var status_url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&fun=updatemixSolventStatus', '',1);?>");
        $.ajax({			
			url : status_url,
			type :'post',

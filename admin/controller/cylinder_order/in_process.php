...
$bradcums[] = array(
	...
	'href' 	=> $obj_general->link($rout,'&mod=index&status=1', '',1),
	'icon' 	=> 'fa-list',
	'class'	=> '',
);
    ...
    <div class="row">
      ...
      <div class="col-lg-12">
        ...
        <!-- <div class="padding-md">
            ...
            <form class="form-horizontal" method="post" data-validate="parsley" action="<?php echo $obj_general->link($rout, '&mod=in_process&order_id='.$_GET['order_id'], '',1); ?>">
                  ...
                  <footer class="panel-footer <?php echo $class; ?>">
                    <div class="row">
                       <div class="col-lg-12">
                        ...
                        <a href="<?php echo $obj_general->link($rout, 'mod=in_process&order_id='.$_GET['order_id'], '',1); ?>" class="btn btn-info btn-sm pull-right" ><i class="fa fa-refresh"></i> Refresh</a>
                       </div> 
                    </div>
                  </footer>                                  
              </section>
                  ...
                  if($total_orders){
					 ...
					 // printr($orders);
						...
						//printr($vender_name);
						  ?>                      
					  	 <tr>
                             ...
                             <td>
                                <input type="button" name="rec<?php echo $f;?>" id="rec<?php echo $f;?>" value="Receive" style="border:0px" class="bg-success" onclick="rec(<?php echo $f;?>,<?php echo $order['order_id']?>)" />
					  ...
					  <?php 
                        ...
                        $pagination->url = $obj_general->link($rout, '&mod=in_process&order_id='.$_GET['order_id'].'&page={page}&limit='.$limit.'&filter_edit=1', '',1);//HTTP_ADMIN.'index.php?rout='.$rout.'&page={page}';
    ...
    <div class="modal-content" style="width: 400px;">
    	<form class="form-horizontal" method="post" name="sform" id="sform" style="margin-bottom:0px;">
              <div class="modal-header">
                ...
                <input type="hidden" name="modalorder_id" id="modalorder_id" value="" />
                 <input type="hidden" name="admin" id="admin" value="<?php echo ADMIN_EMAIL;?>" />
                
                <h4 class="modal-title" id="myModalLabel"> Receive Date For Cylinder Order</h4>
              </div>
              <div class="modal-body">
                   <div class="form-group">
                        <label class="col-lg-3 control-label">Receive Date</label>
                        <div class="col-lg-8">
                        <input type="text" class="combodate form-control" data-format="YYYY-MM-DD" data-required="true" data-template="D MMM YYYY" 
              ...
              <div class="modal-footer">
                ...
                <button type="button" onclick="Receivecylinder()" name="btn_decline" class="btn btn-danger">Receive</button>
              </div>
   		</form>   
    </div>
  </div>
...
function Receivecylinder()
{
	//alert("hi");
	var datetime = $("#datetime").val();
	//alert(datetime);
	var id = $("#modalorder_id").val();
	var adds_url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&fun=updateCyl', '',1);?>");
	//alert(adds_url);
	$.ajax({
				url : adds_url,
				type :'post',

...
if($obj_session->data['LOGIN_USER_TYPE']==1 && $obj_session->data['ADMIN_LOGIN_SWISS']==1) {
	if(isset($_POST['inactive']) || (isset($_GET['status']) && $_GET['status'] == 1) || (isset($_POST['status']) && $_POST['status'] == 1) ){
		$con= " AND mcoi.custom_order_status = 1  AND mcoi.status='0' AND mcoi.quotation_status=0";
		$cond=" AND mco.custom_order_status = 1  AND mco.status='0' AND mcoi.quotation_status=0 ";
		$status = '1';
	}
	elseif(isset($_POST['notsave']) || (isset($_GET['status'])  && $_GET['status']== 2) || (isset($_POST['status']) && $_POST['status'] == 2)){
		$con= " AND mcoi.custom_order_status = 0  AND mcoi.quotation_status=0 ";
		$cond=" AND mco.custom_order_status = 0   AND mcoi.quotation_status=0 ";
		$status = '2';
	}
	else{
		$con= " AND mcoi.custom_order_status = 1  AND mcoi.status='1' AND mcoi.quotation_status=0 ";
		$cond=" AND mco.custom_order_status = 1  AND mco.status='1' AND mcoi.quotation_status=0 ";
		$status = '0';
	}
}
else
{
	$con= " AND mcoi.custom_order_status = 1 AND mcoi.quotation_status=0";
	$cond=" AND mco.custom_order_status = 1 AND mcoi.quotation_status=0 ";	
}
//echo $status;
//echo $con.'<br>'.$cond;
	...
	if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post'])){
		...
		if(!$obj_general->hasPermission('delete',$menuId)){
			...
			foreach($_POST['post'] as $cust_order_id){
				$obj_custom_order->deleteCustomOrder($cust_order_id);
			}
                          ...
                          <div class="col-lg-4">
                           ...
                           <div class="col-lg-4">
                              <div class="form-group">
                                ...
                                <?php							
									$userlist = $obj_custom_order->getUserList();
								?>
                                <div class="col-lg-7">
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table id="quotation-row" class="table b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php
                  $total_custom_order = $obj_custom_order->getTotalCustomOrder($obj_session->data['LOGIN_USER_TYPE'],$obj_session->data['ADMIN_LOGIN_SWISS'],$filter_data,$cond,$add_book_id);
				...
				//die;
                  ...
                  if($total_custom_order!=0){
                      ...
                      $customOrder = $obj_custom_order->getCustomOrders($obj_session->data['LOGIN_USER_TYPE'],$obj_session->data['ADMIN_LOGIN_SWISS'],$option,$filter_data,$add_book_id);		
                           ...
                           <?php if($addedByInfo['country_id'] == 252 ){?>
							 ...
							 <td>
                              ...
                              <?php //if($_SESSION['LOGIN_USER_TYPE']=='1' && $_SESSION['ADMIN_LOGIN_SWISS']=='1'){?>
                                    <td><a class="btn btn-primary btn-xs" id="<?php echo $cust_order['multi_custom_order_id']; ?>" onclick="clone_data(<?php echo $cust_order['multi_custom_order_id']; ?>)" style="background-color:#d18de2;">Clone</a></td>
                              <?php //} ?>
							</tr>
							<?php
							//$f++;

...
if(isset($_GET['custom_order_id']) && !empty($_GET['custom_order_id'])){
	...
	//	printr($custom_order_id);
		...
		$tax_type = $obj_custom_order->CheckCustomOrderTax($data[0]['custom_order_id']);
		  $multi_quation_id = $obj_custom_order->getmulti_quation_id($data[0]['multi_product_quotation_id']);
	//	echo $quotation_id;
	//	printr($data);		
	}
}
...
if($display_status){
	...
	if(isset($_POST['btn_save'])){
		...
		$obj_custom_order->upadteCustomOrder($custom_order_id);
		...
		if(isset($_POST['smail']) && !empty($_POST['smail'])){
			...
			if (!filter_var($gemail, FILTER_VALIDATE_EMAIL)) {
				...
				if(isset($_POST['sscurrency']) && !empty($_POST['sscurrency']) ){
					$getSelCurrecnyData = $obj_custom_order->getSelCurrencyInfo(decode($_POST['sscurrency']));
					$setCurrencyId = 0;
					if($getSelCurrecnyData){
						$setCurrencyId = $obj_custom_order->setCustomOrderCurrency($custom_order_id,$getSelCurrecnyData['currency_code'],$getSelCurrecnyData['price'],1);
					}
				}
			
		    	$obj_custom_order->sendCustomOrderEmail($custom_order_id,$gemail,$setCurrencyId);
	...
	{
		$adminCountryId=$obj_custom_order->getUser($user_id,$user_type_id);
	}
	if(isset($_POST['btn_adddetail'])){
	    $obj_custom_order->insertNote($_POST);
	    if(!empty($_FILES['die_line']['name'][0]))
		    $obj_custom_order->uploadDieline($_POST,$_FILES['die_line'],1);   
		if(!empty($_FILES['art_image']['name'][0]))
		    $obj_custom_order->uploadDieline($_POST,$_FILES['art_image'],2); 
		page_redirect($obj_general->link($rout, '&mod=view&custom_order_id='.encode($custom_order_id).$add_url, '',1)); 
	}
              ...
              <div class="panel-body">
                 ...
                 <form class="form-horizontal" method="post" name="form" id="form" enctype="multipart/form-data">
					  ...
					  if($allow_currency_status && $data[0]['status'] == 1){
						  ...
						  $currencys = $obj_custom_order->getNewCurrencys();
				   // printr($data[0]['currency']);
						  if($currencys){ 
							  ?>
                      ...
                      <div class="col-lg-7" style=" width:50%">
                          ...
                          <div class="form-group">
                           ...
                           <div class="form-group">
                            ...
                            <div class="col-lg-4">
                                <label class="control-label normal-font">
                                <?php echo ucwords($multi_quation_id['multi_quotation_number']);?>
                                </label>
                            </div>
                          </div>
                          <div class="form-group">
					...
					?>
                      <div class="form-group">
								<label class="col-lg-3 control-label" style="width: 10%;">Price (By <?php echo $rush; ?>)</label>
                                          ...
                                          <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        foreach($sdata as $soption){        
														   ...
														   ?>
                                                            ...
                                                            <th>
                                                                    <?php echo $soption['product_name'];?>
                                                                    <?php $code = $obj_custom_order->getproductCode($soption['product_code_id']); 
                                                                    echo '</br></br>Product Code : <span style="color:red;">'.$code['product_code'].'</span>';  ?>
                                                            </th>
                                                            <th><?php echo $skey;
																if($soption['cust_quantity'] != '0')
																	echo '</br></br><b>Order Qty: </b>'.$soption['cust_quantity'] ;
																
																if($soption['client_order_qty'] != '0')
																	echo '</br></br>Client Order Qty:   '.$soption['client_order_qty'] ;
																?></th>
                                                             <td>
											  ...
											  echo '<b>'.($gi+1).' Layer : </b>'.$soption['materialData'][$gi]['material_name'].' : '.(int)$soption['materialData'][$gi]['material_thickness'].'<br>';
            													...
            													<?php   if($soption['discount'] && $soption['discount'] >0.000) {?>
                                                                            <b>Total : </b><?php $pretot= $obj_custom_order->numberFormate((($soption['totalPrice'] / $skey) / $dat['currency_price']),"3"); echo $pretot;?><br />
                                                                            <b>Discount (<?php echo $soption['discount'];?> %) : </b>
            																<?php $predis = $pretot*$soption['discount']/100; 
            																   echo $obj_custom_order->numberFormate($predis,"3");?><br />
                                                                            <b>Final Total : </b>
            																<?php echo $dat['currency'].' '.$obj_custom_order->numberFormate(($pretot-$predis),"3"); ?>
            														<?php }else echo $dat['currency'].' '.
																	        $obj_custom_order->numberFormate((($soption['totalPrice'] / $skey) / $dat['currency_price']),"3");
																	        
																	        if($soption['client_total_price'] != '0')
																                	echo '</br></br><b>Client Rate: </b>  '.$obj_custom_order->numberFormate(($soption['client_total_price'] / $soption['client_order_qty']),"3") ;
																	        ?>
                                                               </td>
                                                                ...
                                                                <td>
																	<?php  //$soption['discount']='12.3';
                                                                        if($soption['discount'] && $soption['discount'] >0.000) {?>
                                                                                <b>Total : </b><?php $tot= $obj_custom_order->numberFormate(($soption['totalPrice'] / $dat['currency_price'] ),"3"); echo $tot;?><br />
                                                                                <b>Discount (<?php echo $soption['discount'];?> %) : </b>
                                                                                	<?php $dis = $tot*$soption['discount']/100; 
                                                                                			echo $obj_custom_order->numberFormate($dis,"3");?><br />
                                                                                <b>Final Total : </b>
                                                                               	 	<?php echo $dat['currency'].' '.($tot-$dis);
																				if($soption['cust_total_price'] != '0.000') 
																				{
																					echo ' <br><br><b>Order Total : </b>'.$soption['cust_total_price'];?><br />
																						<b>Discount (<?php echo $soption['discount'];?> %) : </b>
																					
																					<?php $dis_cus = $soption['cust_total_price']*$soption['discount']/100;
																							 echo $obj_custom_order->numberFormate($dis_cus,"3");?><br />
																								
																				<?php	echo '<b>Order Final Total : </b>'.($soption['cust_total_price']-$dis_cus).'<br />';
                                                                    ...
                                                                    <?php } 
																		  ...
																		  {
                                                                                 echo $dat['currency'].' '.$obj_custom_order->numberFormate(($soption['totalPrice'] / $dat['currency_price'] ),"3");
																				 echo '<br /><br /><b>Order Final Total : </b>'.$soption['cust_total_price'];
																		 }
																		
																		/*if($soption['cust_total_price'] != '0.000')
																				echo '</br></br>'.$dat['currency'].' '.$obj_custom_order->numberFormate(($soption['cust_total_price']),"3"); */?>
                                                                 </td>
                                                                 <?php if($k=='pickup') {
																	  if($data[0]['shipment_country_id']==111) { ?>
                                                  				<td>
																	<?php echo $dat['currency'].' '.$obj_custom_order->numberFormate((($soption['totalPriceWithTax'] / $skey) / $dat['currency_price'] ),"3");?>
                                                                 </td>
                                                                 <td>
																	<?php echo $dat['currency'].' '.$obj_custom_order->numberFormate(($soption['totalPriceWithTax'] / $dat['currency_price'] ),"3");?>
                                                                 </td>
                                                 <?php }  }?>  <td><?php if($soption['cylinder_price']>0) {echo (int)$soption['cylinder_price'];}else '';
												 ?></td>
                                                              ...
                                                              <a class="btn btn-danger btn-sm" id="<?php echo $soption['custom_order_price_id']; ?>" href="javascript:void(0);"><i class="fa fa-trash-o"></i></a>
                                                                </td><?php } //echo $custom_order_id;?>
                                                                
                                                                <td><a class="btn btn-primary btn-sm" onclick="add_details(<?php echo $multi_custom_order_id.','.$soption['custom_order_id'];?>)" id="<?php echo $soption['custom_order_price_id']; ?>" href="javascript:void(0);" >Add Detail</a><br /><br /> 
                                                                <a class="btn btn-primary btn-sm" id="<?php echo $soption['custom_order_price_id']; ?>" href="<?php echo $obj_general->link($rout, 'mod=view_custom_order&custom_order_id='.encode($soption['custom_order_id']).'&order_id='.$_GET['custom_order_id'].'&filter_edit='.$filteredit, '',1);?>" >View Detail</a></td>
                                                                <td>
                                                                 <?php  //[kinjal] (10-9-2016) 
                                                                 $menu_id = $obj_template->getMenuPermission(ORDER_ACCEPT_ID,$obj_session->data['ADMIN_LOGIN_SWISS'],$obj_session->data['LOGIN_USER_TYPE']);
																  ...
																  {
																	  	//[kinjal] edited on 17-4-2017
																  
																		//if($obj_session->data['LOGIN_USER_TYPE']== '1' && $data[0]['custom_order_status'] == '1')
																		//{ 
						...
						//if($data[0]['status'] == 1){
                           ...
                           <?php /*?> <div class="form-group">
                                <div class="col-lg-12"><h4><i class="fa fa-tags"></i> Email/PDF History</h4>
                                	...
                                	<?php
									//$history_data =array();
                                    	$history_data = $obj_custom_order->getEmailHistories($data[0]['multi_custom_order_id']);	  
                                                ...
                                                <?php if($history_data) {
                                                      foreach($history_data as $hdata){
                                                          ...
                                                          <tr>
                                                             ...
                                                             <?php
                                                             if(isset($data[0]['currency']) && $data[0]['currency'] != 'INR'){
                                                                   echo '<td>'.$obj_custom_order->numberFormate($hdata['currency_rate'],"3").'</td>';
                                                             }
                                                             ?>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
    	<form class="form-horizontal" method="post" name="dform" id="dform" style="margin-bottom:0px;" enctype="multipart/form-data">
              ...
              <div class="modal-body">
                ...
                <div class="col-lg-8">                    
                    <div class="media-body">
                            <input type="file" multiple="multiple" name="die_line[]" id="die-line1"  title="<i class='fa fa-folder-open-o'></i> Browse" class="btn btn-sm btn-info m-b-small">
                    </div>                    
              ...
              <div class="form-group">
                <label class="col-lg-4 control-label">Art Work<br/><small class="text-muted">(Only .pdf & .jpg format)</small></label>
                <div class="col-lg-8">                    
                    <div class="media-body">
                            <input type="file" multiple="multiple" name="art_image[]" id="art-image1" title="<i class='fa fa-folder-open-o'></i> Browse" class="btn btn-sm btn-info m-b-small">
                    </div>                    
              ...
              <div class="modal-footer">
                <button type="button" class="btn btn-default btn-sm" data-dismiss="modal">Close</button>
                <button type="submit" name="btn_adddetail" id="btn_adddetail" class="btn btn-primary btn-sm">Add</button>
              </div>
   		</form>   
    </div>
  </div>
...
/*$('#btn_adddetail').click(function(){
	...
	//var formData=$('#dform').serialize();
        ...
        var file_data = $("#die-line").prop("files")[0];$("#myFile1")[0].files[0]
            formData.append("file", file_data)
            formData.append("product_note", $("#product_note").val())
            formData.append("product_special_instruction", $("#product_special_instruction").val())
            formData.append("multi_custom_order_id", $("#multi_custom_order_id").val())
            formData.append("custom_order_id", $("#custom_order_id").val())

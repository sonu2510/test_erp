...
//Start : edit
$edit = '';
if(isset($_GET['cylinder_currency_id']) && !empty($_GET['cylinder_currency_id'])){
	if(!$obj_general->hasPermission('edit',$menuId)){
		$display_status = false;
	}else{
		$cylinder_currency_id = base64_decode($_GET['cylinder_currency_id']);
		$currency = $obj_currency->getCurrency($cylinder_currency_id);
		$edit = 1;
	}
...
if($display_status){
	...
	if(isset($_POST['btn_update']) && $edit){
		$post = post($_POST);
		//printr($post);die;
		$cylinder_currency_id = $currency['cylinder_currency_id'];
		$obj_currency->updateCurrency($cylinder_currency_id,$post);

...
if(isset($_GET['quotation_id']) && !empty($_GET['quotation_id'])){
	if(!$obj_general->hasPermission('view',$menuId)){ 
	    ...
	    $tax_type = $obj_label_quotation->CheckQuotationTax($data[0]['label_quotation_product_id']);
    }
   // printr($data);
}
...
if(!empty($data))
{
	...
	if(isset($_POST['btn_sendemail'])){
		if(isset($_POST['smail']) && !empty($_POST['smail'])){
			...
			if (!filter_var($gemail, FILTER_VALIDATE_EMAIL)) {
			   ...
			   $obj_label_quotation->label_quotation_mail($quotation_id,$gemail,$setCurrencyId,$_POST['sel_currency_sec'],$_POST['currency_rate']);
	...
	//$obj_label_quotation->label_quotation_mail($quotation_id);
if(isset($_POST['btn_gen']))
{
    $obj_label_quotation->saveQuotation($quotation_id);
	$obj_label_quotation->label_quotation_mail($quotation_id,'','','','',ADMIN_EMAIL);
                                                    ...
                                                    ?>
                                                        ...
                                                        //  printr($details);
												           echo '<tr>';
                                                    	              ...
                                                    	              echo ' <td>'.$details['product_name'].','.$details['shape_name'].'<br><br><b>('.$details['printing_effect_detail'].')</b></td>';
                                                    	              echo ' <td>'.$details['volume'].'['.$details['sticker_width'].'X'.$details['sticker_height'].']<b>('.$details['make_name'].')</b></td>';
                                                    	              echo ' <td>'.$details['sheet_name'].'['.$details['sheet_width'].'X'.$details['sheet_height'].']<br><b>No. of Stickers per sheet : </b>'.$details['no_of_sticker_per_sheet'].'</td>';
                                                    	              echo ' <td>'.$obj_label_quotation->numberFormate(($details['price_per_label'] / $details['product_rate']),3).' '.$details['currency_code'].'</td>';
                                                    	              echo ' <td>'.$obj_label_quotation->numberFormate(($details['total_amount']/ $details['product_rate']),3).' '.$details['currency_code'].'</td>';
                                                	       echo '</tr>';
                                                        }
                      ...
                      <div class="form-group">
                        <div class="col-lg-9 col-lg-offset-3"> 
                            <?php if($data[0]['quotation_status']==0){?>
                                <button type="submit" name="btn_gen" id="btn_gen" class="btn btn-primary">Save Quotation</button>
                            <?php } ?>

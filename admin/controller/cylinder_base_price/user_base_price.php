            ...
            <table class="table table-striped b-t text-small">
              <thead>
                <tr>
                  ...
                  <th>Price</th>
                  <th>Gres(Percentage % For Cylinder)</th>
                  <th>Action</th>
                </tr>
              </thead>
              <tbody>
              <?php
              $total_branch = $obj_cylinder->getTotalBranch($filter_data);
			  $pagination_data = '';
			  if($total_branch){
				   	if (isset($_GET['page'])) {
						$page = $_GET['page'];
				  ...
				  $branchs = $obj_cylinder->getBranchs($option,$filter_data);
				 ...
				 // printr($branchs);
				  ...
				  //  printr($branch);
					?>
                   <tr>
                    <td width="20"><input type="checkbox" name="post[]" value="<?php echo $branch['international_branch_id'];?>"></td>
                    <td><?php echo $branch['name'];?></td>
                    <td><?php echo $branch['email'];?></td>
                    <td><?php echo $branch['country_name'];?></td>
                    <td><?php echo $obj_cylinder->getdefaultcurrencyCode($branch['default_curr']);?></td>
                    <td><input type="hidden" name="default_cyli_base_price" value="<?php echo $branch['default_cyli_base_price'];?>" class="form-control input-sm" id="default_cyli_base_price_<?php echo $branch['international_branch_id']; ?>"></td>
                    <td><input type="text" name="gres_cyli" value="<?php echo $branch['gres_cyli'];?>" class="form-control input-sm" id="gres_cyli_<?php echo $branch['international_branch_id']; ?>"></td>
                    <td><a name="btn_edit" class="btn btn-info btn-xs" onclick="updateprice(<?php echo $branch['international_branch_id'];?>)">Update</a></td>
...
<script>
	...
	function updateprice(international_branch_id){
	    var gres_cyli=$("#gres_cyli_"+international_branch_id).val();
	    var default_cyli_base_price=$("#default_cyli_base_price_"+international_branch_id).val();
	    
	   // alert(gres_cyli);
	    //alert(default_cyli_base_price);
	    var url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&fun=update_price', '',1);?>");
			$.ajax({
				type: "POST",
				url: url,					
				data:{international_branch_id:international_branch_id,gres_cyli:gres_cyli,default_cyli_base_price:default_cyli_base_price},

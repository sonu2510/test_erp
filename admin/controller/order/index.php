	...
	if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post'])){
		...
		if(!$obj_general->hasPermission('delete',$menuId)){
			...
			foreach($_POST['post'] as $order_id){
				$obj_order->deleteOrder($order_id);
			}
          ...
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
            	<?php if($obj_general->hasPermission('add',$menuId)){ ?>
   					<a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New Order </a>
                <?php } ?>
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table id="quotation-row" class="table b-t text-small table-hover">
                  ...
                  $total_orders = $obj_order->getTotalOrders($obj_session->data['LOGIN_USER_TYPE'],$obj_session->data['ADMIN_LOGIN_SWISS'],$filter_data);
//printr(count($total_orders));
                  $pagination_data = '';
                  if($total_orders){
                        if (isset($_GET['page'])) {
                            $page = (int)$_GET['page'];
                      ...
                      $orders = $obj_order->getOrders($obj_session->data['LOGIN_USER_TYPE'],$obj_session->data['ADMIN_LOGIN_SWISS'],$option,$filter_data);
					  $f = 1;
                      ...
                      foreach($orders as $order){
						 // printr($order);
						  ?>                      
					  	 <tr>
                             <td><input type="checkbox" name="post[]" value="<?php echo $order['order_id'];?>"></td>	
                             <td width="1%"><?php echo $f;?></td>	 
                             <td>
							 	<a href="<?php echo $obj_general->link($rout, '&mod=view&order_id='.encode($order['order_id']), '',1);?>"><?php echo $order['order_number']; ?><br/>
                             	<small class="text-muted"><?php echo date("d-M-y",strtotime($order['date_added'])); ?></small></a>
                             </td>
                             <td>
                             	<a href="<?php echo $obj_general->link($rout, '&mod=view&order_id='.encode($order['order_id']), '',1);?>">
							 	<?php echo $order['company_name']; ?><br/>
                             	<small class="text-muted"><?php echo $order['email']; ?></small></a>
                             </td>
                             <!--<td><a href="<?php echo $obj_general->link($rout, '&mod=view&order_id='.encode($order['order_id']), '',1);?>"><?php echo $order['total_price']; ?></a></td>-->
                             <td>
                                <a href="<?php echo $obj_general->link($rout, '&mod=view&order_id='.encode($order['order_id']), '',1);?>">
                             	<label class="label   
                             ...
                             <?php
								$postedByData = $obj_multi_quotation->getUser($order['added_by_user_id'],$order['added_by_user_type_id']);
							...
							?>
                             ...
                             <td>	
                                <a href="<?php echo $obj_general->link($rout, 'mod=edit&order_id='.encode($order['order_id']),'',1); ?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>

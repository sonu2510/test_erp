...
if($fun == 'removeInvoice'){
	...
	$data=$obj_invoice->removeInvoice($_POST['sales_invoice_product_id'],$_POST['sales_invoice_id']);
	echo $data;
...
{
	//printr($_POST['value']);
	$obj_invoice->change_qty_per_kg($_POST['inv_color_id'],$_POST['value'],$_POST['n'],$_POST['invoice_status']);
}
...
if($fun=='change_invoice_qty')
{
	//printr($_POST['value']);
	$obj_invoice->change_invoice_qty($_POST['inv_product_id'],$_POST['value'],$_POST['n']);
}
if($fun=='getProductDesc')
{   //echo $_POST['invoice_id'];
	$result = $obj_invoice->viewInvoice('',$_POST['invoice_id'],'','','1');
	$html='';
	$html.='<table border="1" class="tool-row table  b-t text-small">';
	  $html.=$result;
	$html.='<tr><td colspan="8"><a class="label bg-info pull-right" href="'.HTTP_SERVER.'/admin/index.php?route=government_sales_invoice&mod=view&invoice_no='.encode($_POST['invoice_id']).'" target="_blank">Go To invoice</a></td></tr>
	        </table>
	        ';
	echo $html;
}
if($_GET['fun']=='viewDailyStockSalesReport')
 { 
	 ...
	 $post = array('f_date'=>$data->f_date,
    	...
    	//printr($post);die;
      $html = $obj_invoice ->viewDailyStockSalesReport($post);
      
     // printr($html);die;

	 echo $html;
 }
 if($_GET['fun']=='viewDailyStockRegisterReport')
 { 
	 ...
	 $post = array('f_date'=>$data->f_date,
      ...
      $html = $obj_invoice ->viewDailyStockRegisterReport($post);
      $h.= $html;
      //printr($html);die; 

	 echo $h;
 }if($_GET['fun']=='government_sales_report')
 { 
	 ...
	 $post = array('f_date'=>$data->f_date,
      ...
      $html = $obj_invoice ->government_sales_report($post);
      $h.= $html;
      //printr($html);die; 

	 echo $h;
 }
if($_GET['fun'] == 'get_disc')
{
	$disc = $obj_invoice->getProductCdDetails($_POST['productCode_text'],1);
	echo json_encode($disc);
}
if($_GET['fun'] == 'dispatch')
{
	$disc = $obj_invoice->dispatch_product($_POST['invoice_id']);
	
}
if($_GET['fun']=='viewDailyStockSalesReportStateWise')
 { 
    ...
    //	printr($post);die;
      $html = $obj_invoice ->viewDailyStockSalesReportStateWise($post);
       $h.= $html;
     // printr($html);die;

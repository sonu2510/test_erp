...
if(isset($_POST['btn_gen']))
{
	...
	page_redirect($obj_general->link('invoice', '&mod=box_detail&invoice_no='.encode($_POST['added_invoice_id']).'&inv_status=1'.$add_url, '',1));
          ...
          <header class="panel-heading bg-white"> 
            ...
            <?php
			 ...
			 if(($_SESSION['ADMIN_LOGIN_SWISS']=='19' && $_SESSION['LOGIN_USER_TYPE']=='2') || ($_SESSION['ADMIN_LOGIN_SWISS']=='1' && $_SESSION['LOGIN_USER_TYPE']=='1')) { ?>
              <a class="label bg-success" style="margin-left:4px;" onclick="show_dis_list()"><i class="fa fa-trash-o"></i> Generate Invoice</a>
            ...
            <?php 
             ...
             <div class="panel-body">
                        <form class="form-horizontal" method="post" data-validate="parsley" action="<?php echo $obj_general->link($rout, 'mod=cartlist_view&status='.$_GET['status'].$add_url, '',1); ?>">
                  ...
                  <footer class="panel-footer <?php echo $class; ?>">
                    <div class="row">
                       <div class="col-lg-12">
                        ...
                        <a href="<?php echo $obj_general->link($rout, 'mod=cartlist_view&status='.$_GET['status'].$add_url, '',1); ?>" class="btn btn-info btn-sm pull-right" ><i class="fa fa-refresh"></i> Refresh</a>
                       </div> 
                    </div>
                  </footer>                                  
              </section>
            ...
            <div class="col-lg-3 pull-right">	
                ...
                <option value="<?php echo $obj_general->link($rout, '', '',1);?>" selected="selected">--Select--</option>	
                    ...
                    ?>
                            <option value="<?php echo $obj_general->link($rout, 'mod=cartlist_view&status='.$_GET['status'].'&limit='.$display_limit.$add_url, '',1);?>" selected="selected"><?php echo $display_limit; ?></option>				
                    <?php } else { ?>
                            <option value="<?php echo $obj_general->link($rout, 'mod=cartlist_view&status='.$_GET['status'].'&limit='.$display_limit.$add_url, '',1);?>"><?php echo $display_limit; ?></option>
                    <?php } ?>
                    <?php } ?>
                 </select>
           </div>
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
					...
					/* if(isset($_GET['status']))
					 {
					  ...
					  /* if($_SESSION['ADMIN_LOGIN_SWISS']=='1' && $_SESSION['LOGIN_USER_TYPE']=='1' || !isset($_GET['status']))
					   {*/
                           ...
                           <td><a href="<?php echo $obj_general->link($rout, 'mod='.$mod.'&client_id='.encode($order['client_id']).'&stock_order_id='.encode($order['stock_order_id']).''.$cust_cond.'', '',1);?>"><?php echo $order['gen_order_id'];?></a></td>
                           <?php					  
				...
				//printr($total); 
					   ?>                       
							  <td><a href="<?php echo $obj_general->link($rout, 'mod='.$mod.'&client_id='.encode($order['client_id']).'&stock_order_id='.encode($order['stock_order_id']).''.$cust_cond.'','',1);?>"><?php echo dateFormat(4,$order['date_added']);?></a></td>
...
<!--model for dispatch list   sonu  change width 20-4-2017 -->
...
function get_user_dis_data()
{
	...
	var data_url = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajax&fun=get_user_dis_data', '',1);?>");
	$.ajax({
		url : data_url,
		method : 'post',
...
function addInvoiceData()
{
		...
		var data_url = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajax&fun=addInvoiceData', '',1);?>");
		$.ajax({
			url : data_url,
			method : 'post',

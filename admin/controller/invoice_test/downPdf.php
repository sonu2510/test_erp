					...
					<form class="form-horizontal" method="post" name="form" id="form" enctype="multipart/form-data">
					 ...
					 <div class="form-group">
						...
						<?php if($_GET['status']==3) { ?>
							<a class="label bg-info " onclick="barcode(1);" href="javascript:void(0);"><i class="fa fa-print" ></i> IN Lable PDF</a>
						<?php } else
                        {?>
...
function inout_label(status)
{
	...
	$(".note-error").remove();
	var url = '<?php echo HTTP_SERVER.'pdf/inoutpdf_test.php?mod='.encode('inoutpdfshort').'&token='.rawurlencode($_GET['invoice_id']).'&status=';?>' +status+'<?php echo  '&ext='.md5('php');?>&from='+from_box+'&to='+to_box;
	console.log(url);
	window.open(url, '_blank');
}function barcode(status)
{
	var from_box = $("#from_box").val();
	var to_box = $("#to_box").val();
	$(".note-error").remove();
	var url = '<?php echo HTTP_SERVER.'pdf/inoutpdf_test.php?mod='.encode('inoutpdfshort').'&token='.rawurlencode('MzIwNQ==').'&status=1&p_status=2&ext='.md5('php');?>&from='+from_box+'&to='+to_box;
	window.open(url, '_blank');
}

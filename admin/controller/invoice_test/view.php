      ...
      <div class="col-sm-8" style="width:75%">
            <section class="panel">  
             ...
             <a class="label bg-info pdfcls" href="javascript:void(0);"><i class="fa fa-print"></i> PDF</a>
                <a  class="label bg-info pdfclspacking" onclick = "open_detail()"  href="javascript:void(0);"><i class="fa fa-print"></i> Packing PDF</a>
          ...
          <?php /*?>   <a class="label bg-success" href="javascript:void(0);" onclick="csvlink('<?php echo rawurlencode($_GET['invoice_no']);?>','<?php echo $_GET['status'];?>')"><i class="fa fa-print"></i> CSV</a><?php */?>
              <a class="label bg-success" href="javascript:void(0);" onclick="excellink('<?php echo rawurlencode($_GET['invoice_no']);?>','<?php echo $_GET['status'];?>')"><i class="fa fa-print"></i> Excel</a>
                 <a class="label bg-primary sendmailcls" href="javascript:void(0);"><i class="fa fa-envelope"></i> Send Mail</a>
                  ...
                  <?php if($addedByInfo['country_id']=='111' && $_GET['inv_status']!='1'){?>
                   <span style="float:right">
                       <a class="label bg-info " onclick="downloadPdf('<?php echo $_GET['invoice_no'];?>',<?php echo $_GET['inv_status'];?>,<?php echo $_GET['status'];?>);" href="javascript:void(0);"><i class="fa fa-print" ></i> If you not able to download INOUT Label so plz click on this button</a>
                ...
                <span style="float:right">
                      ...
                      <a href="<?php echo $obj_general->link($rout, 'mod=d_test&invoice_no='.$_GET['invoice_no'].'&status=1&inv_status='.$_GET['inv_status'],'',1); ?>"  name="btn_edit" class="btn btn-info btn-xs">INOUT(Test)</a>
                ...
                <?php }?>
                   <span style="float:right">
                                ...
                                <?php if(decode($invoice['transportation'])=='sea'  || $invoice['show_pallet']=='1') {
                                ...
                                <a href="<?php echo $obj_general->link($rout, 'mod=details&invoice_no='.$_GET['invoice_no'].'&price=1&inv_status='.$_GET['inv_status'],'',1); ?>" id="packing_price"  name="btn_edit" class="btn btn-info btn-xs">Packaging Details With Price List</a>
                           ...
                           <?php }
                           if($invoice['done_status']!='1' && $_GET['inv_status']==1){ ?>
                              			  <span style="float:right">
...
<script>
 ...
 jQuery(document).ready(function(){
		...
		$(".pdfcls").click(function(){			
				...
				$(".note-error").remove();
				var url = '<?php echo HTTP_SERVER.'pdf/invoicepdf_test.php?mod='.encode('invoice_test').'&token='.rawurlencode($_GET['invoice_no']).'&status='.rawurlencode($_GET['status']).'&ext='.md5('php');?>';
...
function excellink(id,status){
		var url = '<?php echo HTTP_SERVER.'word/invoice_excel_test.php?mod='.encode('invoice_test').'&ext='.md5('php');?>&token='+id+'&status='+status;
		window.open(url, '_blank');
	return false;
}
...
{
	$(".note-error").remove();
  // var open1 = '<?php echo HTTP_SERVER.'admin/index.php?route=invoice_test&mod=details&invoice_no='.$_GET['invoice_no'].'&price=1&inv_status'.$_GET['inv_status'];?>';
   var url = '<?php echo HTTP_SERVER.'pdf/detailspdf_test.php?mod='.encode('details_test').'&token='.rawurlencode($_GET['invoice_no']).'&price=1&ext='.md5('php');?>';

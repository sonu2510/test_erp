...
if(isset($_FILES['file']['name']) && $_FILES['file']['name'] != '' && $_FILES['file']['error'] == 0){
   // print_r($_FILES);	die;	
		...
		if($ext == 'pdf' || $ext == 'PDF' || $ext == 'Pdf')
		{
			...
			$upload_path = DIR_UPLOAD.'admin/return_doc/php-pdf-merge-master/example/';				
			$file_name = $_FILES['file']['name'];
			$filetemp = $_FILES['file']['tmp_name'];	
			
			$upload_file_path = $upload_path.''.$_POST['invoice_id'].':'.$_POST['invoice_no'].'/second.pdf';
		...
		//	$obj_template->sendDispatchOrderEmail($post,'3',$_POST['admin'],'','',$on_ship='1');
			...
			$return_array = array(
				'ext' => 'pdf',
				'name' => DIR_UPLOAD.'admin/return_doc/php-pdf-merge-master/example/'.$_POST['invoice_id'].':'.$_POST['invoice_no'].'/'.$file_name 
			);
		//	printr($return_array);die;

...
if($display_status) {
	...
	if(isset($_POST['action']) && ($_POST['action'] == "active" || $_POST['action'] == "inactive") && isset($_POST['post']) && !empty($_POST['post']))
	{
		...
		$obj_product_measurement->updateStatus($status,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	
	}else if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post'])){
	 //printr($_POST['post']);die;
		$obj_product_measurement->updateStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
                 ...
                 <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New Template Measurement </a>
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php	
                  $total_unit = $obj_product_measurement->getTotalProduct($filter_data);
                  $pagination_data = '';
                  if($total_unit){
                        if (isset($_GET['page'])) {
                            $page = (int)$_GET['page'];
                      ...
                      $products = $obj_product_measurement->getProducts($option,$filter_data);
...
<script type="application/javascript">
	$('input[name=status]').change(function() {
		...
		//alert(status_value);
		var status_url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&ajax=UpdateProductMeasurementStatus', '',1);?>");
        //alert(status_url);
		$.ajax({			
			url : status_url,
			type :'post',

...
$total_transport = $obj_transport->getTotalRollTransport();
$pagination_data = '';
?>
<section id="content">
          	...
          	<div class="table-responsive">
                <table class="table table-striped b-t text-small table-hover">
                  ...
                  if($total_transport){
                      ...
                      $option = array(
                            'sort'  => 'product_roll_transport_id',
                            'order' => 'ASC',
                            'start' => ($page - 1) * LISTING_LIMIT,
                            'limit' => LISTING_LIMIT
                      );	
                      $transports = $obj_transport->getRollTransports($option);
                      foreach($transports as $transport){ 
                        ?>
                        <tr>
                          <td><?php echo $transport['from_kgs'];?></td>
                          <td><?php echo $transport['to_kgs'];?></td>
                          <td><?php echo $transport['price_kgs'];?></td>
                          <td>
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&transport_id='.encode($transport['product_roll_transport_id']), '',1); ;?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                           </td>
                        </tr>
                        <?php
                      }

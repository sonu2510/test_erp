            ...
            <?php } ?>
                    <?php if($obj_general->hasPermission('add',$menuId)){ 
            				if($obj_session->data['LOGIN_USER_TYPE'] != 1)
            				{
            					...
            					{?><?php if(!isset($_GET['pro']) && isset($_GET['status']) && $_GET['status']==0) {?>
                                        ...
                                        <a class="label bg-info" href="<?php echo $obj_general->link($rout, 'mod=add_sea_order&s_no='.encode($permission).'&status=0'.$add_url, '',1);?>">
                                        <i class="fa fa-plus"></i> New Sea Stock Order </a>
                                    <?php }
                                        elseif(isset($_GET['pro']) && $_GET['pro']=='digi')
                                        { ?>
                                            <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add_digi&s_no='.encode($permission).'&pro=digi'.$add_url, '',1);?>">
                                            <i class="fa fa-plus"></i> New Digital Order </a>
                                 <?php  }
                                                ...
                                                <div class="form-group">
                                					...
                                					<select class="form-control" name="filter_ib_user_name">
                                    					...
                                    					<?php foreach($ib_userlist as $ib_user) { ?>
                                        				    <?php if(isset($filter_ib_user_name) && $filter_ib_user_name == $ib_user['user_id']) { ?>
									...
									<div class="form-group">
                                		...
                                		<div class="col-lg-6">                                                            
                                            <select class="form-control" name="filter_by_shipment">
												...
												<option value="By Air" <?php if(isset($filter_by_shipment) &&  $filter_by_shipment == 'By Air') {echo 'selected="selected"'; } ?>>By Air</option>
												<option value="By Sea" <?php if(isset($filter_by_shipment) &&  $filter_by_shipment == 'By Sea') {echo 'selected="selected"'; } ?>>By Sea</option>
											</select>
                                		</div>
                              		</div>
									</div>
                                		...
                                		<div class="col-lg-6">     
                                            ...
                                            <select class="form-control" name="filter_user_name">
                                                ...
                                                <?php foreach ($userlist as $user) { ?>
                                                    <?php if ($splitdata && $splitdata[0] == $user['user_type_id'] && $splitdata[1] == $user['user_id']) { ?>
    							...
    							<div class="col-lg-4">
    							    <div class="form-group">
                                      <label class="col-lg-3 control-label">Product Code</label>
                                        <?php $productcodes = $obj_template->getActiveProductCode(array());?>
                                        <div class="col-lg-4">
    ...
    <div class="modal-content">
    	...
    	<form class="form-horizontal" method="post" name="sform" id="form_dis" style="margin-bottom:0px;">
              ...
              <div class="modal-body">
               ...
               <div class="modal-body" id="dispatch_list_div">
...
<!--model for Add roll detaiils-->
<div class="modal fade" id="show_roll" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog" style="width:85%;">
    <div class="modal-content">
        	<div class="table-responsive">
    	<form class="form-horizontal" method="post" name="roll_form" id="form_roll" style="margin-bottom:0px;">
        	<input type="hidden" name="curr_date" id="curr_date" value="<?php echo date("Y-m-d");?>"  />
        	<input type="hidden" name="custom_order_id" id="custom_order_id" value=""  />
              <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                     <h4 class="modal-title u_title" id="myModalLabel"></h4>
              </div>
             	<div class="table-responsive">
              <div  id="roll_list_div">
                 ...
                 <div class="modal-footer">
                    <button type="button" class="btn btn-default btn-sm" data-dismiss="modal">Close</button>
                    <button type="button"  name="data_roll" id="data_roll" onclick="addRollData()" class="btn btn-warning">Done</button>
...
{	
	...
	$('#loading').show();	
	var order_status_url = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajaxt&fun=updatestockorderstatus'.$add_url, '',1);?>");
	$.ajax({
							 ...
							 if(item.product_id!=6){
								...
								else
								{
									...
									else
									{
									  ...
									  //  alert(ship_country);
									    if(ship_country==42 || ship_country==214 || ship_country==14 || ship_country==155 || ship_country==189 || ship_country==172 ){
									        	$("#netweight_"+i).val(item.net_weight); 
        										$("#boxweight_"+i).val(item.box_weight);
        										$("#box_qty_"+i).val(item.box_qty);
        										var dis_qty = $("#dis_qty_id_"+i).val();
...
function getrollnetweight(custom_order_id,order_no)
{
    var roll_no = $("#cust_roll_qty_no_"+custom_order_id).val();
   // alert(roll_no);
   $('#custom_order_id').val(custom_order_id);
     var html='';
       	 html +='<div class="form-group">';
       	 html +='<div class="col-lg-9 div_len">';
       	 html +='<table class="table table-bordered"><thead><tr>';
		 	 html +='<th >Roll No </th>';
		 	 html +='<th >Net Weight(In Kgs)</th>';
			 html +='<th>Box Weight(In Kgs)</th>'; 
		     html +='<th>Box Qty</th>';
			...
			html +='<tbody>';
		if(roll_no!=0){	
		    for(var i=1; i<=roll_no; i++){
		    	html +='<tr>';
    			html +='<td><b>'+i+' . </b></td>';
    			html +='<td><input type="text" value="" name="roll['+i+'][Rnetweight]" id="Rnetweight" class="form-control"  placeholder="Net Weight"/> </td>';
    			 html +='<td><input type="text" value="" name="roll['+i+'][RboxWeight]" id="RboxWeight" class="form-control" placeholder="Box Weight" /></td>';
    		     html +='<td><input type="text" value="1" name="roll['+i+'][Rbox_qty]" id="Rbox_qty" class="form-control"  placeholder="Box Qty"/></td>';
    			 html +='<td><input type="text" value="1" name="roll['+i+'][Rtotalbox]" id="Rnetweight" class="form-control"  placeholder="Total Box"/></td>';
    	...
    	$(".u_title").html("Add Roll Details For Order No <b> "+order_no+" </b> " );	
    
    	$("#roll_list_div").html(html);	
     $("#show_roll").modal('show');
	
}
function addRollData()
{
    var formData = $("#form_roll").serialize();
  var custom_order_id= $('#custom_order_id').val();
  	var addRollDataurl = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajax&fun=addRollData', '',1);?>");
	$.ajax({
		url : addRollDataurl,
		method : 'post',
		data : {formData:formData},
		success: function(response){
		       console.log(response);
	        $('#cust_roll_details_'+custom_order_id).val(response);
			 $("#show_roll").modal('hide');
		},
			error: function(){
				return false;	
			}
               ...
               $.each(data, function (i,item) {
                ...
                if(div_attr=='cust')
                {
                  ...
                  else
                  {
                     ...
                     if(ship_country==42 || ship_country==214 || ship_country==155 || ship_country==189 || ship_country==172 ){

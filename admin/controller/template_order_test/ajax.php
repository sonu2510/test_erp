    	 	    ...
    	 	    //if(!empty($detailslistdigital)){
                    ...
                    $icon="<i class='fa fa-folder-open-o'></i>";
    	 	    	$response .= '<td style="width:160px"> 
            							...
            							<a class="file-input-wrapper btn btn btn-sm btn-info m-b-small">'.$icon.' Upload Die line<input type="file" name="die_line" id="die-line" title="Upload Die line" class="btn btn-sm btn-info m-b-small"></a>
            							<br>
            		...
            		$response .= '</td>';
    	 	   // }<input type="file" name="die_line" id="die-line" title=" '.$icon.'Browse" class="btn btn-sm btn-info m-b-small">
	...
	if(!empty($result))
	{
		...
		$response .= '<div style="width:80%; position:initial;margin:0 auto;"  >';
			...
			
			{
			     ...
			     if($data['stock_print']=='Digital Print' || ($data['quotation_status']==1)){
...
if($fun == 'get_user_dis_data')
{
								...
								//	printr($s_data['product_template_order_id']);
									...
									//	printr($dis_qty);
												...
												$result.='<tr id="stock-row-'.$s_data['template_order_id'].'">
															...
															{
																$result.='<td><input type="text" style="display:none;" name="rate_'.$s_data['template_order_id'].'" value="0" class="form-control validate[required]"/></td>';
															}
															else 
															{
		...
		//	printr($data);
			...
			//{
    			...
    			{	
    				...
    				$cust_o_id['quotation_status_'.$custom_order_id['multi_custom_order_id']]=$custom_order_id['quotation_status'];
	...
	//printr($new_data);
			...
			//printr($currency_price);
					$trans = $k;
					$result.='<div class="table-responsive"><div class="form-group">
								<label class="col-lg-3 control-label" style="width: 10%;">Price ('.$trans.')</label> 
								<div class="col-lg-9 div_len">
									<section class="panel">
									  <div class="table-responsive">
										<table class="table table-striped b-t text-small">
										  ...
										  <tbody>';
											...
											//	printr($soption);
												//foreach($reference_no as $r_no){
												 ...
												 if($soption['quotation_status']==1){
                            					     $stock_print='  <b>(Digital Print </b>)';
                            					     $digital_color=$obj_template->GetdigitalColorName($soption['total_color']);
                            					     
                            					   //  printr($soption['total_color']);
                            					     $plate_note='Including Plate price ';
                            					       $d_color=' <br><b>Digital Printing With </b>'.$digital_color.'<br> <b> Front Side : </b> '.$soption['front_color']. ' Color <br><b> Back Side :</b>'.$soption['back_color'].' Color.';
                            					 }else{
                            					     $stock_print='';
                            					      $d_color='';
                            					      $plate_note='';
													...
													$result.='	
																   ...
																   <input type="hidden" name="cust_digital_print_color_'.$soption['custom_order_id'].'" value="'.$soption['total_color'].'"/>
												 ...
												 $multi_custom_order_no = '"'.$cust_o_id[$soption['multi_custom_order_id']].'"';
													$result.='<tr id="custom-row-'.$soption['custom_order_price_id'].'">
														...
														<input type="hidden" name="reference_no_'.$soption['custom_order_id'].'" value="'.$reference_no['ref_o_no_'.$soption['multi_custom_order_id']].'"/>																		
																	...
																	<td>'.$soption['product_name'].''. $stock_print.' '.$d_color;
															...
															//		printr($soption['totalPrice']);
																	if($soption['quotation_status']!=1){
																	    ...
																	    $per_pouch=$soption['gress_pouch_price'];
																	     $result.=$currency.' '.$soption['gress_pouch_price'].' <br>'.$plate_note;
																	}
																	...
																	if($soption['quotation_status']!=1){
																	    ...
																	    $result.=$currency.' '.$obj_invoice->numberFormate(($soption['totalPrice']),"3");
																	}
																	$result.='</td>';
																	if($k=='pickup') {
																		if($shipment_country_id==111)
...
if($fun == 'getProductSize') {
	...
	$data = $obj_template->getProductSize($product_id,$zipper_id,$make_id,$spout_pouch);
	$response = '';	
		$response .='<select id="size" name="size" class="form-control validate[required]" onchange="getColor()"><option value="">Select Size</option>';
	if($data){	
		foreach($data as $item){
				//$response .= '<div class="checkbox " style="float: left;  width: 50%;"><label>';
					...
					$response .= '<option value="'.encode($item['size_master_id']).'" volume = "'.$item['volume'].'">';
					if($item['volume']!=0)
...
if($fun == 'getColor')
{
   $data = $obj_template->getColorName($_POST['product_id'],$_POST['zipper_id'],$_POST['make_id'],$_POST['volume']); 
   $response='';
...
if($fun == 'checktemplatelistsea'){
	...
	if($detailslist)
    	...
    	$response .= '<table class="table table-bordered">';
    	$response .= '<thead>';
    	$response .= '<tr style="font-weight: 600;">';   
    	$response .= '<th rowspan="2"><b>Quantity in Stock</b></th>';    
    	$response .= '<th colspan="2"><b>Last 4 Month</b></th>';
    	$response .= '<th colspan="2"><b>Previous 4 Month</b></th>';    	
    	$response .= '<th colspan="2"><b>Previous 4 Month</b></th>';    	
    	$response .= '<th style="width: 170px;" rowspan="2"><b>Quantity</b></th>';
    	$response .= '<th rowspan="2"><b>Expected Delivery Date</b></th>';
    	$response .= '<th rowspan="2"><b>Action</b></th>';
    	$response .= '</tr>'; 
    	$response .= '<tr style="font-weight: 600;">';   
       	$response .= '<th ><b>No of Customer</b></th>';
       	$response .= '<th ><b>Quantity</b></th>';
    	$response .= '<th ><b>No of Customer</b></th>';
       	$response .= '<th ><b>Quantity</b></th>';
        $response .= '<th ><b>No of Customer</b></th>';
        $response .= '<th ><b>Quantity</b></th>';
    	...
    	$rack_qty = $obj_rack_master->getRackQty($product_code_id,$user_type_id,$user_id,'','');
    	
    	$qty_with_customer_4month = $obj_rack_master->getDispatchQtywithCustomer($product_code_id,$user_type_id,$user_id,1,4);
    	$qty_with_dis_qty_4month = $obj_rack_master->getDispatchQtywithCustomer($product_code_id,$user_type_id,$user_id,0,4);

    	$qty_with_customer_8month = $obj_rack_master->getDispatchQtywithCustomer($product_code_id,$user_type_id,$user_id,1,8);
    	$qty_with_dis_qty_8month = $obj_rack_master->getDispatchQtywithCustomer($product_code_id,$user_type_id,$user_id,0,8);

    	$qty_with_customer_12month = $obj_rack_master->getDispatchQtywithCustomer($product_code_id,$user_type_id,$user_id,1,12);
    	$qty_with_dis_qty_12month = $obj_rack_master->getDispatchQtywithCustomer($product_code_id,$user_type_id,$user_id,0,12);

    	$qty_with_customer_3month = $obj_rack_master->getDispatchQtywithCustomer($product_code_id,$user_type_id,$user_id,1,3);
    	$qty_with_dis_qty_3month = $obj_rack_master->getDispatchQtywithCustomer($product_code_id,$user_type_id,$user_id,0,3);
		 ...
		 $rm_qty=0;$total_rm_qty=0;
    		...
    		$response .= '<tr id='.$details['product_template_size_id'].' style="font-weight: 600;">';    	
    		$response .= '<td>'. $total_rm_qty. '</td>';
    		$response .= '<td>'.$qty_with_customer_4month.'</td>';
    		$response .= '<td>'.$qty_with_dis_qty_4month.'</td>';
    		$response .= '<td>'.$qty_with_customer_8month.'</td>';
    		$response .= '<td>'.$qty_with_dis_qty_8month.'</td>';
    		$response .= '<td>'.$qty_with_customer_12month.'</td>';
    		$response .= '<td>'.$qty_with_dis_qty_12month.'</td>';
    	...
    	$response .= '<div style="display:none">';
           ...
           $response .='<td> <label class="col-lg-3 control-label"><span class="required">*</span></label>
    	   				<input type="hidden" name="stock_price_comp'.$i.'" id="stock_price_comp'.$i.'" value='.$getpricedata['stock_price_compulsory'].'>
                        <input type="text" name="quantity'.$i.'" id="quantity'.$i.'" value="" style="width:100px"  
    					class="form-control validate[required,custom[onlyNumberSp],minSize[4]]" /> 
    					</td>';

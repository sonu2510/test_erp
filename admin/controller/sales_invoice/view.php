...
if(isset($_POST['btn_sendemail'])){
    ...
    $url = HTTP_SERVER.'pdf/salesinvoicepdf.php?mod='.encode('salesinvoice').'&token='.$_GET['invoice_no'].'&status='.rawurlencode($_GET['status']).'&ext='.md5('php');
  ...
  <section class="main padder">
    ...
    <div class="row">
      ...
      <div class="col-sm-8" style="width:75%">
            ...
            <a class="label bg-success" href="javascript:void(0);" onclick="excellink('<?php echo $invoice_no;?>','<?php echo $_GET['status'];?>')"><i class="fa fa-print"></i> Excel</a> <?php */?>
               <a class="label bg-success" href="javascript:void(0);" onclick="excellink('<?php echo rawurlencode($_GET['invoice_no']);?>','<?php echo $_GET['status'];?>','0')"><i class="fa fa-print"></i> Excel</a>
                 <?php if($addedByInfo['country_id']=='155') {?>
				 <a class="label bg-info pdfclsspan" href="javascript:void(0);"><i class="fa fa-print"></i> PDF IN SPANISH</a>
		         <a class="label bg-success" href="javascript:void(0);" onclick="excellink('<?php echo rawurlencode($_GET['invoice_no']);?>','<?php echo $_GET['status'];?>','1')"><i class="fa fa-print"></i> Excel IN SPANISH</a>
		        
				 <?php } /*?>
                 <a class="label bg-success" href="javascript:void(0);" id="excel_link"><i class="fa fa-print"></i> Excel</a>
                 <?php */?><a class="label bg-primary sendmailcls" href="javascript:void(0);"><i class="fa fa-envelope"></i> Send Mail</a>
                 </span><br /><br />                
              </header>
              ...
              <div class="panel-body">                                        	
                  ...
                  <b></b>
                    ...
                    <div>
                     <?php $html =$obj_invoice->viewInvoiceForIB($_GET['status'],$invoice_no );
					 //$html =$obj_invoice->viewInvoiceForIBSpanish($_GET['status'],$invoice_no );
					echo $html;?>
                ...
                <div class="form-group">
                     <div class="col-lg-9 col-lg-offset-3"> 
                           <?php $link='mod=index&is_delete='.$_GET['is_delete'].$add_url;if($_GET['status']==2)
                           { $link= 'mod=view&status=1&invoice_no='.$_GET['invoice_no'].$add_url; }?>
    	...
    	<form class="form-horizontal" method="post" name="sform" id="sform" style="margin-bottom:0px;">
              ...
              <div class="modal-body">
                   <div class="form-group">
                        ...
                        <div class="col-lg-8">
                             <input type="text" name="smail" id="email_input" placeholder="Email" value="<?php echo $addedByInfo['email'];?>" class="form-control validate[required,custom[email]]">
                        </div>
                     </div>               </div>
...
<script>
 jQuery(document).ready(function(){
		...
		$(".pdfcls").click(function(){			 
				...
				$(".note-error").remove();
				var url = '<?php echo HTTP_SERVER.'pdf/salesinvoicepdf.php?mod='.encode('salesinvoice').'&token='.$_GET['invoice_no'].'&status='.rawurlencode($_GET['status']).'&ext='.md5('php').'&n=0';?>';
				window.open(url, '_blank');
			return false;
		});
		$(".pdfclsspan").click(function(){			 
		
				$(".note-error").remove();
				var url = '<?php echo HTTP_SERVER.'pdf/salesinvoicepdf.php?mod='.encode('salesinvoice').'&token='.$_GET['invoice_no'].'&status='.rawurlencode($_GET['status']).'&ext='.md5('php').'&n=1';?>';
				window.open(url, '_blank');
			return false;
		});
		$('.getfile').click(
...
function excellink(id,status,n){
		//console.log(id+'==='+status);
		var url = '<?php echo HTTP_SERVER.'word/invoice_sale.php?mod='.encode('invoice_sales').'&ext='.md5('php');?>&token='+id+'&status='+status+'&n='+n;
		window.open(url, '_blank');
	return false;
}

function csvlink(id,status,n){
		var url = '<?php echo HTTP_SERVER.'word/csv_invoice.php?mod='.encode('invoice').'&ext='.md5('php');?>&token='+id+'&status='+status+'&n='+n;
		window.open(url, '_blank');
	return false;
}

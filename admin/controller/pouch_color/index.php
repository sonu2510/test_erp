...
if(isset($_POST['action']) && ($_POST['action'] == "active" || $_POST['action'] == "inactive") && isset($_POST['post']) && !empty($_POST['post']))
{
	...
	if(!$obj_general->hasPermission('delete',$menuId)){
		...
		//printr($_POST['post']);die;
		$obj_color->updateStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
}
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
                 ...
                 <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New Pouch Color </a>
          	...
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  ...
                  if($total_color){
                      ...
                      $colors = $obj_color->getColors($option,$filter_data);
					  ...
					  foreach($colors as $color){ 
					      ...
					      //if($color['make_id']!=''){
					      //   printr($color['make_id']); 
					  
					  
					     if(isset($color['make_id'])&& !empty($color['make_id']) ){
					         
					    	  $make_name= $obj_color->getMakeName($color['make_id']);
                        ...
                        ?>
                        <tr <?php echo ($color['status']==0) ? 'style="background-color:#FADADF" ' : '' ; ?>>
                          <td><input type="checkbox" name="post[]" value="<?php echo $color['pouch_color_id'];?>"></td>
                         
                          <td><?php echo $color['color'];?></td>
                          
                           <?php 
                             $color_category = $obj_color->getColorCategory($color['color_category']);
                            // printr($color_category);
                             ?>
                         
                           <td><?php echo $color_category['color_name'];?></td>
                           <td><?php echo $make_name;?></td>
                          <td>
                          
                          	<div data-toggle="buttons" class="btn-group">
                                <label class="btn btn-xs btn-success <?php echo ($color['status']==1) ? 'active' : '';?> "> <input type="radio" 
                                 name="status" value="1" id="<?php echo $color['pouch_color_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label>                                   
                                <label class="btn btn-xs btn-danger <?php echo ($color['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                    name="status" value="0" id="<?php echo $color['pouch_color_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label> 
                            </div>
                          
                          </td>
                          <td>	
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&color_id='.encode($color['pouch_color_id']),'',1); ?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>

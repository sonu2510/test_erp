...
if(isset($_POST['btn_filter'])){
	...
	$filter_edit = 1;
	$class ='';	
	if(isset($_POST['filter_machine'])){
		$filter_machine=$_POST['filter_machine'];		
	}else{
		$filter_machine='';
	}	
...
{	if(!$obj_general->hasPermission('edit',$menuId)){
		...
		$obj_machine->updateStatus($status,$_POST['post']);
...
else if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post']))
{
	if(!$obj_general->hasPermission('delete',$menuId)){
		...
		$obj_machine->updateStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
}
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
                 ...
                 <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New Machine Name </a>
                      ...
                      <div class="row">
                          <div class="col-lg-4">
                              <div class="form-group">
                                ...
                                <div class="col-lg-7">
                                  <input type="text" name="filter_machine" value="<?php echo isset($filter_machine) ? $filter_machine : '' ; ?>" placeholder="Machine" id="input-name" class="form-control" />
                                </div>
                              </div>                             
                          </div>
                          <div class="col-lg-4">                              
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php	
				  $total_machine = $obj_machine->getTotalMachine($filter_data);
				  $pagination_data = '';
                  if($total_machine){
                        if (isset($_GET['page'])) {
                            $page = (int)$_GET['page'];
                      ...
                      $machines = $obj_machine->getMachines($option,$filter_data);
                   
					  
					  foreach($machines as $mach){ 
                        ?>
                        <tr <?php echo ($mach['status']==0) ? 'style="background-color:#FADADF" ' : '' ; ?>>
                          <td><input type="checkbox" name="post[]" value="<?php echo $mach['machine_id'];?>"></td>
                                    
                          <td><?php echo $mach['machine_name'];?></td>
                          <td>
                          
                          	<div data-toggle="buttons" class="btn-group">
                                <label class="btn btn-xs btn-success <?php echo ($mach['status']==1) ? 'active' : '';?> "> <input type="radio" 
                                 name="status" value="1" id="<?php echo $mach['machine_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label>                                   
                                <label class="btn btn-xs btn-danger <?php echo ($mach['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                    name="status" value="0" id="<?php echo $mach['machine_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label> 
                            </div>
                          
                          </td>
                          <td>	
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&machine_id='.encode($mach['machine_id']),'',1); ?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
...
<script type="application/javascript">
	...
	$('input[type=radio][name=status]').change(function() {
        ...
        $.ajax({			
			url : status_url,
			type :'post',
			data :{machine_id:machine_id,status_value:status_value},
			success: function(){

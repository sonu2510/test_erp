...
{	if(!$obj_general->hasPermission('edit',$menuId)){
		...
		$obj_oxy_silica_detail->Updateoxy_silicaStatus($status,$_POST['post']);
...
else if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post']))
{
	...
	if(!$obj_general->hasPermission('delete',$menuId)){
		...
		$obj_oxy_silica_detail->Updateoxy_silicaStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
}
...
<section id="content">
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  ...
                  <tbody>
                  	<?php
					$count = $obj_oxy_silica_detail->getcount();
					$pagination_data = '';
					if($count){
							if (isset($_GET['page'])) {
								$page = (int)$_GET['page'];
						...
						$results = $obj_oxy_silica_detail->getvalue();	
						foreach($results as $result){
						
						?>
						<tr <?php echo ($result['status']==0) ? 'style=background-color:#FADADF' : '' ; ?>>
                          <td><input type="checkbox" name="post[]" value="<?php echo $result['oxy_silica_id'];?>"></td> 
						  <td><?php echo $result['prodct_name'];?></td>
						  ...
						  <td>
                          	<div data-toggle="buttons" class="btn-group">
                                <label class="btn btn-xs btn-success <?php echo ($result['status']==1) ? 'active' : '';?> "> <input type="radio" 
                                 name="status" value="1" id="<?php echo $result['oxy_silica_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label>                                   
                                <label class="btn btn-xs btn-danger <?php echo ($result['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                    name="status" value="0" id="<?php echo $result['oxy_silica_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label> 
                           </div>
                          </td>
						  <td>
                          	<a href="<?php echo $obj_general->link($rout,'mod=add&oxy_silica_id='.encode($result['oxy_silica_id']),'',1)?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
						   </td>
						</tr>
						<?php
						}
 ...
 $('input[name=status]').change(function(){
	var oxy_silica_id = $(this).attr('id');
	var oxy_silica_status = this.value;
	var storezo_url = getUrl("<?php echo $obj_general->ajaxLink($rout,'&mod=ajax&ajaxfun=UpdateStatus','',1);?>");
	 $.ajax({
		 url : storezo_url,
		 type : 'post',
		 data : {oxy_silica_id:oxy_silica_id,oxy_silica_status:oxy_silica_status},

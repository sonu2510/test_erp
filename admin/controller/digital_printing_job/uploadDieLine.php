...
if(isset($_FILES['file']['name']) && $_FILES['file']['name'] != '' && $_FILES['file']['error'] == 0){
		...
		if($ext == 'pdf'){
			...
			$upload_path = DIR_UPLOAD.'admin/pdfDigitalPrinting/';				
			...
			$return_array = array(
				'ext' => 'pdf',
				'name' => HTTP_UPLOAD.'admin/pdfDigitalPrinting/' .$file_name 
			);			
			echo json_encode($return_array);
		}else if($validateImageExt){
			$upload_path = DIR_UPLOAD.'admin/DigitalPrinting/';				
			$file_name = $_FILES['file']['name'];
			$filetemp = $_FILES['file']['tmp_name'];
			$upload_file_path = $upload_path.$file_name;		
			...
			$return_array = array(
				'ext' => 'img',
				'name' => HTTP_UPLOAD.'admin/DigitalPrinting/' .$file_name 
			);
			echo json_encode($return_array);			
		}else{
			echo 0;

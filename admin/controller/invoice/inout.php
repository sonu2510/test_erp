...
<section id="content">
  ...
  <div class="panel-body" id="print_div">
    <div class="">
     <div class="form-group" id="in_out" style="width:600px; font-size:8px"> 
...
<script>
 jQuery(document).ready(function(){
		...
		$(".pdfcls").click(function(){			
				...
				$(".note-error").remove();
				var url = '<?php echo HTTP_SERVER.'pdf/inoutpdf.php?mod='.encode('inout').'&token='.rawurlencode($_GET['invoice_no']).'&status='.rawurlencode($_GET['status']).'&ext='.md5('php');?>';
	...
	jQuery(window).load(function () {
		  jQuery("#in_out").children().each(function(n, i) {
    if($(this).height()>23)
	  he = he+$(this).height(); 
	   if(he>800)
	   {
...
function wordlink(id){
		var url = '<?php echo HTTP_SERVER.'word/invoice_inout.php?mod='.encode('invoice').'&status='.$_GET['status'].'&ext='.md5('php');?>&token='+id;
		window.open(url, '_blank');
	return false;
}
...
function excelfile(id,status){
	//debugger;
		var url = '<?php echo HTTP_SERVER.'word/invoice_inout_excel.php?mod='.encode('invoice').'&status='.$_GET['status'].'&ext='.md5('php');?>&token='+id;
		window.open(url, '_blank');
	return false;
}
...
{
	window.location ='<?php echo HTTP_SERVER;?>admin/index.php?route=invoice&mod=downPdf&invoice_id='+invoice_no+'&status='+status+'&inv_status='+inv_status;
	//console.log(url);
//	window.open(url, '');
}
</script>	

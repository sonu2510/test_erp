            	...
            	<section class="panel">
                  ...
                  <div class="panel-body">
                    <form name="form_list" id="form_list" method="post">
                        ...
                        <div>
                        <?php  $html =$obj_invoice->viewConsolidatedSheet($invoice_no);
								echo $html;?>                    
                            </div>
...
$(".pdfcls").click(function(){			
				$(".note-error").remove();
				var url = '<?php echo HTTP_SERVER.'pdf/consolpdf.php?mod='.encode('consol').'&token='.rawurlencode($_GET['invoice_no']).'&price='.$_GET['price'].'&ext='.md5('php');?>';
...
function excelfile(id){
		var url = '<?php echo HTTP_SERVER.'word/consol.php?mod='.encode('invoice').'&price='.$_GET['price'].'&ext='.md5('php');?>&token='+id;
		window.open(url, '_blank');
	return false;
}
//var count = $('#tr').length;

...
if(isset($_GET['user_id']) && !empty($_GET['user_id'])){
	...
	$user = $obj_permission->getUserData(1,$user_id,'user','user_id');
	if(!$user){
...
if(isset($_POST['btn_save'])){
	$post = post($_POST);
	$insert_id = $obj_permission->addPermission(1,$user['user_id'],$post);
	$obj_session->data['success'] = ADD;
	page_redirect($obj_general->link($rout, '', '',1));
}

if($display_status){
	$selColoum = ' admin_menu_id, name, controller, page_name';
	$menu_data = $obj_permission->getMenuData($selColoum);

...
if($display_status){
	...
	if(isset($_POST['btn_save'])){
		$post = post($_POST);		
		//printr($post);die;
		$insert_id = $obj_cylinder->addCylinder($post,$obj_session->data['ADMIN_LOGIN_SWISS'],$obj_session->data['LOGIN_USER_TYPE']);
	...
	if(isset($_POST['btn_update']) && $edit){
		$post = post($_POST);
		//printr($post);die;
		$cylinder_id = $cylinder['product_cylinder_vendor_id'];
		$post['type'] = $cylinder['type'];
		if($cylinder['price'] == $post['price']){
			$obj_cylinder->updateCylinder($cylinder_id,$post,$obj_session->data['ADMIN_LOGIN_SWISS'],$obj_session->data['LOGIN_USER_TYPE']);
		}else{
			$obj_cylinder->addCylinder($post,$obj_session->data['ADMIN_LOGIN_SWISS'],$obj_session->data['LOGIN_USER_TYPE']);
		}
          ...
          <div class="panel-body">
            <form class="form-horizontal" method="post" name="form" id="form" enctype="multipart/form-data">
              ...
              <div class="form-group">
                ...
                <div class="col-lg-4">
                	<?php
					if(isset($cylinder['type']) && $cylinder['type'] == 1){
						echo "India";
					}else{
						echo "International";
					}

...
if(isset($_POST['btn_filter'])){
	...
	$filter_edit = 1;
	$class ='';	
	if(isset($_POST['filter_industry'])){
		$filter_industry=$_POST['filter_industry'];		
	}else{
		$filter_industry='';
	}	
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
                 ...
                 <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New Industry </a>
                      ...
                      <div class="row">
                          <div class="col-lg-4">
                              <div class="form-group">
                                <label class="col-lg-5 control-label">Industry Name</label>
                                <div class="col-lg-7">
                                  <input type="text" name="filter_industry" value="<?php echo isset($filter_industry) ? $filter_industry : '' ; ?>" placeholder="Industry" id="input-name" class="form-control" />
                                </div>
                              </div>                             
                          </div>
                          <div class="col-lg-4">                              
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  <thead>
                    <tr>
                      ...
                      <th class="th-sortable <?php echo ($sort_order=='ASC') ? 'active' : ''; ?> ">
                      		Industry Name
                            <span class="th-sort">
                            	<a href="<?php echo $obj_general->link($rout, 'sort=industry'.'&order=ASC', '',1);?>">
                                <i class="fa fa-sort-down text"></i>
                                <a href="<?php echo $obj_general->link($rout, 'sort=industry'.'&order=DESC', '',1);?>">
                  ...
                  <tbody>
                  <?php
                  $total_industry = $obj_industry->getTotalIndustry($filter_data);
                  $pagination_data = '';
                  if($total_industry){
                        if (isset($_GET['page'])) {
                            $page = (int)$_GET['page'];
						 ...
						 if (isset($_GET['sort'])) {
                            ...
                            $sort_option = 'enquiry_industry_id';
                        }
                      ...
                      $industrys = $obj_industry->getIndustrys($option,$filter_data);
                      //printr($materials);die;
					  
					  foreach($industrys as $industry){ 
                        ?>
                        <tr>
                          <td><input type="checkbox" name="post[]" value="<?php echo $industry['enquiry_industry_id'];?>"></td>
                          <td>
						  	<?php echo $industry['industry'];?>
                          </td>
                          <td>
                          	<div data-toggle="buttons" class="btn-group">
                                <label class="btn btn-xs btn-success <?php echo ($industry['status']==1) ? 'active' : '';?> ">
                                	<input type="radio" name="status" value="1" id="<?php echo $industry['enquiry_industry_id']; ?>">
                                 	<i class="fa fa-check text-active"></i>Active
                                </label>
                                     
                                <label class="btn btn-xs btn-danger <?php echo ($industry['status']==0) ? 'active' : '';?> ">
                                	<input type="radio" name="status" value="0" id="<?php echo $industry['enquiry_industry_id']; ?>">
                                    <i class="fa fa-check text-active"></i>Inactive
                                </label> 
                            </div>
                          </td>
                          <td>
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&industry_id='.encode($industry['enquiry_industry_id']), '',1); ;?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                           </td>
                        </tr>
                        <?php
                      }
                        ...
                        $pagination->total = $total_industry;
...
<script type="application/javascript">
	...
	$('input[type=radio][name=status]').change(function() {
		...
		var status_url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&fun=updateIndustryStatus', '',1);?>");
        $.ajax({			
			url : status_url,
			type :'post',
			data :{industry_id:industry_id,status_value:status_value},
			success: function(){
				$("#loading").hide();

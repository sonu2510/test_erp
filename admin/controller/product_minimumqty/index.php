...
if($obj_session->data['ADMIN_LOGIN_SWISS']!= '1' && $obj_session->data['LOGIN_USER_TYPE'] != '1')
{
	...
	$branchs = $obj_qty->getBranchs('','',$obj_session->data['LOGIN_USER_TYPE'],$obj_session->data['ADMIN_LOGIN_SWISS']);
	foreach($branchs as $branch)
	{
		page_redirect($obj_general->link($rout, 'mod=branch_minqty&branch_id='.encode($branch['international_branch_id']).'','',1));
	}
}
          ...
          <form name="form_list" id="form_list" method="post">
           <input type="hidden" id="action" name="action" value="" /> 
            ...
            <table class="table table-striped b-t text-small">
              ...
              <tbody>
              <?php
              $total_branch = $obj_qty->getTotalBranch($filter_data,$obj_session->data['LOGIN_USER_TYPE'],$obj_session->data['ADMIN_LOGIN_SWISS']);
			  $pagination_data = '';
			  if($total_branch){
				   	if (isset($_GET['page'])) {
						$page = $_GET['page'];
				  ...
				  $branchs = $obj_qty->getBranchs($option,$filter_data,$obj_session->data['LOGIN_USER_TYPE'],$obj_session->data['ADMIN_LOGIN_SWISS']);
				  if($branchs) {
				  foreach($branchs as $branch){ 
					//printr($branch);?>
                    <tr>
                      <td><a href="<?php echo $obj_general->link($rout, 'mod=branch_minqty&branch_id='.encode($branch['international_branch_id']).'','',1);?>"><?php echo $branch['name'];?></a></td>
                      <td><a href="<?php echo $obj_general->link($rout, 'mod=branch_minqty&branch_id='.encode($branch['international_branch_id']).'','',1);?>"><?php echo $branch['email'];?></a></td>
                       <td><a href="<?php echo $obj_general->link($rout, 'mod=branch_minqty&branch_id='.encode($branch['international_branch_id']).'','',1);?>"><?php echo $obj_qty->getCountryName($branch['country_id']);?></a></td>
                      <td><a href="<?php echo $obj_general->link($rout, 'mod=branch_minqty&branch_id='.encode($branch['international_branch_id']).'','',1);?>"><?php echo $obj_qty->getdefaultcurrencyCode($branch['default_curr']);?></a></td>
                      </tr>
                    <?php
				  }
				  }else{

...
if(isset($_POST['action']) && ($_POST['action'] == "active" || $_POST['action'] == "inactive") && isset($_POST['post']) && !empty($_POST['post']))
{
	if(!$obj_general->hasPermission('edit',$menuId)){
		...
		$obj_attendance_type->updateStatus($status,$_POST['post']);
	...
	if(!$obj_general->hasPermission('delete',$menuId)){
		...
		$obj_attendance_type->updateStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
}
          ...
          <form name="form_list" id="form_list" method="post">
           <input type="hidden" id="action" name="action" value="" />
            ...
            <table class="table table-striped b-t text-small">
              ...
              <tbody>
              <?php
              $total_attendance = $obj_attendance_type->getTotalDepartment();
			  //echo $total_attendance;die;
			  $pagination_data = '';
			  if($total_attendance){
				   	if (isset($_GET['page'])) {
						$page = $_GET['page'];
				  ...
				  $attendance_types = $obj_attendance_type->getDepartments($option);
				  foreach($attendance_types as $attendance){ 
					?>
                    <tr>
                      <td><input type="checkbox" name="post[]" value="<?php echo $attendance['attendance_types_id'];?>"></td>
                      <td><?php echo $attendance['type_name'];?></td>
                      <td><label class="label   
                        <?php echo ($attendance['status']==1)?'label-success':'label-warning';?>">
                        <?php echo ($attendance['status']==1)?'Active':'Inactive';?>
                        </label>
                      </td>
                      <td>
                      		<a href="<?php echo $obj_general->link($rout, 'mod=add&attendance_types_id='.encode($attendance['attendance_types_id']), '',1); ;?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                       </td>
                    </tr>
                    <?php
				  }
					...
					$pagination->total = $total_attendance;

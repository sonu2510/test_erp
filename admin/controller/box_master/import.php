	...
	if(isset($_FILES['file']['name']) && $_FILES['file']['name'] != '' && $_FILES['file']['error'] == 0){
		...
		else
		{
			if ($_FILES['file']['size'] > 0) { 
				...
				//printr($handle);
				$invoice_data = $obj_boxmaster->InsertCSVData($handle);
				$obj_session->data['success'] = 'Invoice successfully Added!';
			}
			$filename = $_FILES['file']['name'];
			$filetemp = $_FILES['file']['tmp_name'];
			$path 	 = DIR_UPLOAD.'admin/box_master_data/';
			$f_nm=$path_parts['filename'];
			if(file_exists(DIR_UPLOAD."admin/box_master_data/".$_FILES['file']['name'])) 
			{
				@unlink("box_master_data/$filename");

...
if($display_status) {
	...
	if(isset($_POST['action']) && ($_POST['action'] == "active" || $_POST['action'] == "inactive") && isset($_POST['post']) && !empty($_POST['post']))
	{
		...
		if(!$obj_general->hasPermission('edit',$menuId)){
			...
			//printr($_POST['post']);
			$obj_courier_account_number->updateStatus($status,$_POST['post']);
			//die;
		...
		if(!$obj_general->hasPermission('delete',$menuId)){
			...
			$obj_courier_account_number->updateStatus(2,$_POST['post']);
			$obj_session->data['success'] = UPDATE;
			page_redirect($obj_general->link($rout, '', '',1));
		}
	}
...
$total_couriers = $obj_courier_account_number->getTotalcouriers($filter_data);
$pagination_data = '';
?>
<section id="content">
          ...
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
                ...
                <?php if($obj_general->hasPermission('add',$menuId)){ ?>
   							<a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New Account no </a>
                    <?php }
					if($obj_general->hasPermission('edit',$menuId)){ ?>
          	...
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  ...
                  if($total_couriers){
                      ...
                      $couriers = $obj_courier_account_number->getcouriers($option,$filter_data);
			
                      foreach($couriers as $courier){ 
                   //    printr(($courier['c_status']));  ?>
                        <tr <?php echo ($courier['c_status']==0) ? 'style="background-color:#FADADF" ' : '' ; ?>>
                          <td><input type="checkbox" name="post[]" value="<?php echo $courier['courier_account_number_id'];?>"></td>
                          <td><?php echo $courier['first_name'].' '.$courier['first_name'];?></td>
                          <td><?php echo $courier['country_name'];?></td>
                          <td><?php echo $courier['courier_name'];?></td>
                          <td><?php echo $courier['company_name'];?></td>
                          <td ><?php echo $courier['account_number'];?></td>
                          <td>
                          	
                           		<div data-toggle="buttons" class="btn-group">
                                	<label class="btn btn-xs btn-success <?php echo ($courier['status']==1) ? 'active' : '';?> "> <input type="radio" 
                                    name="status" value="1" id="<?php echo $courier['courier_account_number_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label>
                                     
                                	<label class="btn btn-xs btn-danger <?php echo ($courier['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                    name="status" value="0" id="<?php echo $courier['courier_account_number_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label> 
                                </div>
                          
                           </td>
                          <td>
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&courier_account_number_id='.encode($courier['courier_account_number_id']).'&filter_edit='.$filter_edit, '',1); ;?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                           </td>
                        </tr>
                        <?php
                      }
...
<script type="application/javascript">
	...
	$('input[type=radio][name=status]').change(function() {
		...
		alert($(this).attr('id'));
		var courier_account_number_id=$(this).attr('id');
		var status_value = this.value;
		
		var status_url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&fun=updateCourierStatus', '',1);?>");
        $.ajax({
			
			url : status_url,
			method :'post',
			data :{courier_account_number_id:courier_account_number_id,status_value:status_value},
			success: function(){

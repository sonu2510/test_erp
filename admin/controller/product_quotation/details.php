...
if(isset($_GET['quotation_id']) && !empty($_GET['quotation_id'])){
	if(!$obj_general->hasPermission('edit',$menuId)){
		...
		$getData = ' product_quotation_id, added_by_user_id, added_by_user_type_id, customer_name, shipment_country_id, quotation_number, quotation_type, quantity_type, product_id, product_name, printing_option, printing_effect, height, width, gusset, layer, currency, currency_price, cylinder_price, customer_gress_percentage,pq.status,quotation_status,valve_price';
      ...
      <div class="col-sm-10">
            ...
            <section class="panel">  
                ...
                <form class="form-horizontal" method="post" name="form" id="form" enctype="multipart/form-data">
					...
					//  printr($quotation_packing_transport);die;
					  ?>
                      <div class="form-group">
											   ...
											   <?php
												  foreach($sdata['zipper_option'] as $optionKey=>$soption){
													  echo '<tr>';
														...
														echo '<td>'.$sdata['transport_price'].'</td><td>';
														if(isset($soption['spout_price'])){
																echo '<b>Spout  : </b>'.$soption['spout_price'].'<br>';
															}
															if(isset($soption['accessorie_price'])){
																echo '<b>Acc  : </b>'.$soption['accessorie_price'].'<br>';
															}
																echo '<b>Zipper  : </b>'.$soption['zipper_price'].'<br>
																<b>Valve Price :</b>'.$data['valve_price'].'</td>';
                                                   ...
                                                   <?php
                                                      foreach($adata['zipper_option'] as $aoptionKey=>$aoption){
                                                         ...
                                                         echo '<tr>';
															...
															echo '<td>'.$aoption['courier_charge'].'</td><td >';
															if(isset($aoption['spout_price'])){
																echo '<b>Spout  : </b>'.$aoption['spout_price'].'<br>';
															}
															if(isset($aoption['accessorie_price'])){
																echo '<b>Acc  : </b>'.$aoption['accessorie_price'].'<br>';
															}
																echo '<b>Zipper  : </b>'.$aoption['zipper_price'].'<br>
																<b>Valve Price :</b>'.$data['valve_price'].'</td>';
                                                   ...
                                                   <?php
                                                      foreach($pdata['zipper_option'] as $poptionKey=>$poption){
                                                          echo '<tr>';
															...
															echo '<td>'.$pdata['courier_charge'].'</td><td>';
															if(isset($poption['spout_price'])){
																echo '<b>Spout  : </b>'.$poption['spout_price'].'<br>';
															}
															if(isset($poption['accessorie_price'])){
																echo '<b>Acc : </b>'.$poption['accessorie_price'].'<br>';
															}
																echo '<b>Zipper  : </b>'.$poption['zipper_price'].'<br>
																<b>Valve Price :</b>'.$data['valve_price'].'</td>';

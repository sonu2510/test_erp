...
//	$filter_layer = $obj_session->data['filter_data']['layer'];
	...
	//$filter_option = $obj_session->data['filter_data']['option'];
...
if($obj_session->data['LOGIN_USER_TYPE']==1 && $obj_session->data['ADMIN_LOGIN_SWISS']==1) {
	if(isset($_POST['inactive']) || (isset($_GET['status']) && $_GET['status'] == 1) || (isset($_POST['status']) && $_POST['status'] == 1) ){
		$con= "AND quotation_status = 1  AND status='0'";
	...
	elseif(isset($_POST['notsave']) || (isset($_GET['status'])  && $_GET['status']== 2) || (isset($_POST['status']) && $_POST['status'] == 2)){
		$con= "AND quotation_status = 0  ";
		$cond="AND pq.quotation_status = 0  ";
		$status = '2';
	}
	else{
		$con= "AND quotation_status = 1  AND status='1'";
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
            	<?php if($obj_general->hasPermission('add',$menuId)){ ?>
   					<a class="label bg-primary" href="<?php $nrout = "multi_product_quotation" ; echo $obj_general->link($nrout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New Quotation </a>
                <?php } ?>
          ...
          <div class="panel-body">
           ...
           <div class=" pull-left">
                 ...
                 <!--   <span class="badge" style="background-color:#fcf8e3;">&nbsp;</span> <span class="m-r-small">Active</span>
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table id="quotation-row" class="table b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php
                  $total_quotation = $obj_quotation->getTotalQuotation($obj_session->data['LOGIN_USER_TYPE'],$obj_session->data['ADMIN_LOGIN_SWISS'],$filter_data,$con);
				 // echo $total_quotation;
                  $pagination_data = '';
                  if($total_quotation){
                        if (isset($_GET['page'])) {
                            $page = (int)$_GET['page'];
                      ...
                      foreach($quotations as $quotation){ 
						...
						if($postedByData){  
                            ...
                            <?php if($quotation['quotation_status']==0){ //for not saved?>
                            	<tr id="quotation-row-<?php echo $quotation['product_quotation_id']; ?>" style="background-color:#f2dede">
                            <?php } else { //for inactive?>
                            	<tr id="quotation-row-<?php echo $quotation['product_quotation_id']; ?>" <?php echo ($quotation['status']==0) ? 'style="background-color:#fcf8e3" ' : '' ; ?>>
                            <?php } ?>
							
                            
                              <td><input type="checkbox" name="post[]" value="<?php echo $quotation['product_quotation_id'];?>"></td>	
							  <td width="1%"><?php echo $slNo++;?></td>
							  <td>
								<a href="<?php echo $obj_general->link($rout, '&mod=view&quotation_id='.encode($quotation['product_quotation_id']).'&filter_edit='.$filter_edit, '',1);?>"><?php echo $quotation['quotation_number'];?>
							 ...
							 <td>
                 			 <a href="<?php echo $obj_general->link($rout, '&mod=view&quotation_id='.encode($quotation['product_quotation_id']).'&filter_edit='.$filter_edit, '',1);?>">             	 
							  ...
							  <td>
                                ...
                                <?php /*
									<small class="text-muted"><?php echo $quotation['valve_txt'].', '.$quotation['zipper_txt'];?></small>
								*/ ?>
                                </a>
							  </td>
                              ...
                              <td>
                              	<div data-toggle="buttons" class="btn-group">
                                    ...
                                    name="status" value="1" id="<?php echo $quotation['product_quotation_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label>
                                     
                                	<label class="btn btn-xs btn-danger <?php echo ($quotation['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                    name="status" value="0" id="<?php echo $quotation['product_quotation_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label>
                                	</div>
                              </td>
                              ...
                              <?php if($obj_session->data['LOGIN_USER_TYPE']==1 && $obj_session->data['ADMIN_LOGIN_SWISS']==1) { ?>
                                  ...
                                  <td class="delete-quot">
                                    <a class="btn btn-danger btn-sm" id="<?php echo $quotation['product_quotation_id']; ?>" href="javascript:void(0);"><i class="fa fa-trash-o"></i></a>
                                    </td>
                                    <td>
                                    <a class="btn btn-primary btn-sm" target="_blank" href="<?php echo $obj_general->link('order', '&mod=add&quotation_no='.encode($quotation['quotation_number']), '',1);?>">Place Order</a>
                                  </td>
                              <?php } ?>

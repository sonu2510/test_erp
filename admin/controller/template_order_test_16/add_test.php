			  ...
			  ?>
                	<form class="form-horizontal" method="post" name="form" id="order-form" enctype="multipart/form-data">
                      ...
                      <div class="form-group" id="digital_print_div2" style="display: none">
                          <label class="col-lg-3 control-label">Front color</label>
                          <div class="col-lg-1">
                  ...
                  <div class="form-group" id="digital_print_div1" style="display: none">
                          <label class="col-lg-3 control-label">Back color</label>
                          <div class="col-lg-1">
                               <div class="input-group">
                                  <span class="input-group-btn">
...
function autoComplete(){
	...
	function hi_con(product_id)
	{
		...
		//alert(template_id);
		var product_url = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajax_test&fun=displaycountry', '',1);?>");
		...
		$.ajax({
			...
			if(defaultcountry == val[i].country_id)
			{
        		...
        		var product_url = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajax_test&fun=displaytranspotation', '',1);?>");
	...
	$("#transport_details").change(function()
	{
		...
		//alert(transport);
				//alert(country_id);
		var add_product_url = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajax_test&fun=checktemplatelist', '',1);?>");
	...
	{
		var remove_template_url = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajax_test&fun=removeTemplate', '',1);?>");
		$.ajax({
   ...
   left:$('#panel-order').offset().left
	...
	if( $('#keyword').val()!='')
	{
		if(quantity>0 && cond)
		{
			var addtocart_url = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajax_test&fun=addtocart', '',1);?>");
			$.ajax({
					//url : addtocart_url,
            		//method : 'post',
					...
					console.log(response);
						...
						var refreshcart_url = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajax_test&fun=GetOrderList', '',1);?>");
						$.ajax({
								url : refreshcart_url,
								method : 'post',
								data : {},
...
$(document).click(function(){
		...
		$("#product_code").keyup(function(event){
		 ...
		 {	
			 ...
			 if(event.keyCode != 40 && event.keyCode != 38 )
			 {		
				 var product_code_url = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajax_test&fun=product_code', '',1);?>");
				 $("#loading").css("visibility","visible");
				 $.ajax({
				   type: "POST",

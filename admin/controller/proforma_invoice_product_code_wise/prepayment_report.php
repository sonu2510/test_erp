...
<section id="content">
  <section class="main padder">
    <div class="clearfix">
      <h4><i class="fa fa-list"></i> Prepayment  Report</h4>
    </div>
    <div class="row">
  ...
  jQuery(document).ready(function(){
	...
	{ 
		...
		if($("#form").validationEngine('validate')){ 
			var get_price_url = getUrl("<?php echo $obj_general->ajaxLink($rout, 'mod=ajax&fun=getprepaymentreport', '',1);?>");
					...
					success: function(response){
					    ...
					    { 
					        excelData = 'data:application/excel;charset=utf-8,' + encodeURIComponent(response);	
								 $('<a></a>').attr({
											'id':'downloadFile',
											'download': 'PrePayment Report.xls',
											'href': excelData,
											'target': '_blank'
									}).appendTo('body');

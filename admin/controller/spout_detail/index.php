...
{	if(!$obj_general->hasPermission('edit',$menuId)){
		...
		$obj_spout->UpdatespoutStatus($status,$_POST['post']);
...
else if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post']))
{
	...
	if(!$obj_general->hasPermission('delete',$menuId)){
		...
		$obj_spout->UpdatespoutStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
}
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> <span><?php echo $display_name;?> Listing</span>
          	<span class="text-muted m-l-small pull-right">         			
               <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>" ><i class="fa fa-plus"></i> New spout </a>
                ...
                <table class="table b-t text-small table-hover">
                  ...
                  <tbody>
					...
					if($count){
						...
						$results = $obj_spout->getvalue();	
						foreach($results as $result){
						
						?>
						<tr <?php echo ($result['status']==0) ? 'style=background-color:#FADADF' : '' ; ?>>
                          <td><input type="checkbox" name="post[]" value="<?php echo $result['spout_id'];?>"></td> 
						  <td><?php echo $result['spout_name'];?></td>
						  ...
						  <td>
                          	<div data-toggle="buttons" class="btn-group">
                                <label class="btn btn-xs btn-success <?php echo ($result['status']==1) ? 'active' : '';?> "> <input type="radio" 
                                 name="status" value="1" id="<?php echo $result['spout_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label>                                   
                                <label class="btn btn-xs btn-danger <?php echo ($result['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                    name="status" value="0" id="<?php echo $result['spout_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label> 
                           </div>
                          </td>
						  <td>
                          	<a href="<?php echo $obj_general->link($rout,'mod=add&spout_id='.encode($result['spout_id']),'',1)?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
						   </td>
						</tr>
						<?php
						}

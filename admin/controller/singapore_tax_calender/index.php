				  ...
				  if($tax_total){
                     ...
                     // echo ($tax['status']); 
                       ...
                       <tr <?php  echo ($tax['status']==0) ? 'style="background-color:#FADADF" ' : '' ; ?>>
                          <td><input type="checkbox" name="post[]" value="<?php echo $tax['tax_sin_calender_id'];?>"></td>
                          ...
                          <td>
                          	...
                          	<div data-toggle="buttons" class="btn-group">
                                <label class="btn btn-xs btn-success <?php echo ($tax['status']==1) ? 'active' : '';?> "> <input type="radio" 
                                 name="status" value="1" id="<?php echo $tax['tax_sin_calender_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label>                                   
                                <label class="btn btn-xs btn-danger <?php echo ($tax['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                    name="status" value="0" id="<?php echo $tax['tax_sin_calender_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label> 
                            </div>
                          
                          </td>
                          <td>	
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&tax_sin_calender_id='.encode($tax['tax_sin_calender_id']),'',1); ?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                           ...
                           {
						    ?>
                            <td> <input type="button" name="send_mail<?php echo $i;?>" id="send_mail<?php echo $i;?>" value="Send Mail"  class="btn btn-success" onclick="acceptmail(<?php echo $i;?>,<?php echo $tax['tax_sin_calender_id'];?>)" /></td>
...
<style>
 ...
 <script type="application/javascript">
	   ...
	   $('input[type=radio][name=status]').change(function() {
		...
		//alert(status_tax);
		var status_url = getUrl("<?php echo $obj_general->ajaxLink($rout, '&mod=ajax&fun=updateAciveStatus', '',1);?>");
        $.ajax({			
			url : status_url,
			type :'post',
...
function acceptmail(id,tax_sin_calender_id)
{	
	//alert(tax_sin_calender_id);
	$(".note-error").remove();
	...
	$.ajax({
		...
		method : 'post',
		data : {tax_sin_calender_id : tax_sin_calender_id,adminEmail:adminEmail},
		success: function(response){
			//alert(response);

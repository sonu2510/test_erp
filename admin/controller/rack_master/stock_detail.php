...
if($display_status){
	...
	if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post'])){
		if(!$obj_general->hasPermission('delete',$menuId)){
			...
			page_redirect($obj_general->link($rout, '&mod=stock_detail&stock_id='.$_GET['stock_id'], '',1));
		}
	}
                ...
                <header class="panel-heading bg-white">
                    ...
                    <span class="text-muted m-l-small pull-right">
            			<?php if($obj_general->hasPermission('edit',$menuId)){ ?>
                             <a class="label bg-danger" onclick="formsubmitsetaction('form_list','delete','post[]','<?php echo DELETE_WARNING;?>')">
                 	...
                 	<div class="panel-body" id="result_div">
                		<form name="form_list" id="form_list" method="post">
                           <input type="hidden" id="action" name="action" value="" />
                            <div class="table-responsive"><!--b-t text-small-->
                                <table id="quotation-row" class="table table-hover">
                                  <thead>
                                    <tr>
                                      <th>Sr. No.</th> 
                                      <th>Product Name</th>
						 ...
						 <?php 
						 $slNo=1;$i=1;
						 $stock_data = $obj_rack_master->getdispatchdetail($user_id,$user_type_id,$stock_id);
					//	 printr($stock_data);
						 if(isset($stock_data) && !empty($stock_data)){
							...
							<tr>
                              ...
                              <td><?php echo $slNo++;?><br><input type="checkbox" name="post[]" value="<?php echo $stock['stock_id'];?>" /></td>
                              <td>
                        ...
                        ?>
                            ...
                            <?php 
							//printr($stock_data[0]['row']);
							$rowcol=$stock_data[0]['row'].'-'.$stock_data[0]['column_name'];
								  $goods_id=$stock_data[0]['goods_id'];
							?>            

...
if(isset($_GET['sort'])){
	...
	$sort= 'pq.multi_product_quotation_id';
}
...
if($obj_session->data['LOGIN_USER_TYPE']==1 && $obj_session->data['ADMIN_LOGIN_SWISS']==1) {
	if(isset($_POST['inactive']) || (isset($_GET['status']) && $_GET['status'] == 1) || (isset($_POST['status']) && $_POST['status'] == 1) ){
		$con= "AND mpq.quotation_status = 1  AND mpq.status='0'";
		$cond="AND pq.quotation_status = 1  AND pq.status='0' ";
		$status = '1';
	}
	elseif(isset($_POST['notsave']) || (isset($_GET['status'])  && $_GET['status']== 2) || (isset($_POST['status']) && $_POST['status'] == 2)){
		$con= "AND mpq.quotation_status = 0  ";
		$cond="AND pq.quotation_status = 0  ";
		$status = '2';
	}
	else{
		$con= "AND mpq.quotation_status = 1  AND mpq.status='1'";
		$cond="AND pq.quotation_status = 1  AND pq.status='1' ";
		$status = '0';
	}
}
else
{
	$con= "AND mpq.quotation_status = 1";
	$cond="AND pq.quotation_status = 1 ";	
}
          ...
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
            	<?php if($obj_general->hasPermission('add',$menuId)){ ?>
   					<a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New Template Quotation </a>
                <?php } ?>
                      ...
                      <div class="row">
                        <div class="col-lg-4">
                              <div class="form-group">
                                <label class="col-lg-5 control-label">Template No</label>
                                <div class="col-lg-7">
           ...
           <div class="col-lg-3 pull-right">	
                ...
                <option value="<?php echo $obj_general->link($rout, '&status='.$status, '',1);?>" selected="selected">--Select--</option>	
                    ...
                    ?>
                            ...
                            <option value="<?php echo $obj_general->link($rout, 'limit='.$display_limit.'&status='.$status, '',1);?>" selected="selected"><?php echo $display_limit; ?></option>				
                    <?php } else { ?>
                            <option value="<?php echo $obj_general->link($rout, 'limit='.$display_limit.'&status='.$status, '',1);?>"><?php echo $display_limit; ?></option>
                    <?php } ?>
                    <?php } ?>
                 </select>
           </div>
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table id="quotation-row" class="table b-t text-small table-hover">
                  <thead>
                    <tr>
                      ...
                      <th class="th-sortable <?php echo ($sort_order=='ASC') ? 'active' : ''; ?> ">
                      		Template No.
                            <span class="th-sort">
                            	<a href="<?php echo $obj_general->link($rout, 'sort=date_added'.'&order=ASC', '',1);?>">
                                	<i class="fa fa-sort-down text"></i>
                                    
                                <a href="<?php echo $obj_general->link($rout, 'sort=date_added'.'&order=DESC', '',1);?>">
                  ...
                  <tbody>
                  <?php
                  $total_quotation = $obj_quotation->getTotalQuotation($obj_session->data['LOGIN_USER_TYPE'],$obj_session->data['ADMIN_LOGIN_SWISS'],$filter_data,$cond);
                  $pagination_data = '';
                  if($total_quotation!=0){
                      ...
                      $quotations = $obj_quotation->getQuotations($obj_session->data['LOGIN_USER_TYPE'],$obj_session->data['ADMIN_LOGIN_SWISS'],$option,$filter_data);		
					  ...
					  foreach($quotations as $quotation){ 
						...
						if($postedByData){  
                            ...
                            <?php } ?>
                              <td><input type="checkbox" name="post[]" value="<?php echo $quotation['multi_product_quotation_id'];?>"><?php //echo $quotation['product_quotation_id']; ?></td>	
							  <td width="1%"><?php echo $slNo++;?></td>
							  <td>
								<a href="<?php echo $obj_general->link($rout, '&mod=view&quotation_id='.encode($quotation['multi_product_quotation_id']).'&filter_edit='.$filter_edit, '',1);?>"><?php echo $quotation['multi_quotation_number'];?>
							 ...
							 <td>
                 			 <a href="<?php echo $obj_general->link($rout, '&mod=view&quotation_id='.encode($quotation['multi_product_quotation_id']).'&filter_edit='.$filter_edit, '',1);?>">             	 
                              ...
                              <td><?php if($quotation['profit_type']==0)echo 'Rich';else if($quotation['profit_type']==1) echo 'Poor';else echo 'More Poor'; ?></td>
                              <?php if(isset($status) && $status != 2){?>
                              <td>
                              	<div data-toggle="buttons" class="btn-group">
                                	...
                                	<label class="btn btn-xs btn-danger <?php echo ($quotation['status']==0) ? 'active' : '';?> "> <input type="radio" name="status" value="0" id="<?php echo $quotation['multi_product_quotation_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label>
                                	</div>
                              </td>
                              <?php }?>
                              <?php if($obj_session->data['LOGIN_USER_TYPE']==1 && $obj_session->data['ADMIN_LOGIN_SWISS']==1) { ?>
                              
                                  <td class="delete-quot"><?php //echo $quotation['multi_product_quotation_id']; ?>
                                    ...
                                    <td>
                                    <a class="btn btn-primary btn-sm" target="_blank" href="<?php echo $obj_general->link($rout, '&mod=stocktemplate&quotation_no='.encode($quotation['multi_product_quotation_id']), '',1);?>">Stock Template</a>
                                  </td>
                              <?php } ?>
                        ...
                        $pagination->url = $obj_general->link($rout,'&page={page}&limit='.$limit.'&status='.$status.'&filter_edit=1', '',1);//HTTP_ADMIN.'index.php?rout='.$rout.'&page={page}';

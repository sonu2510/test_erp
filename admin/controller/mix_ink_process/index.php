...
{	if(!$obj_general->hasPermission('edit',$menuId)){
		...
		$obj_mix_ink_process->updateStatus($status,$_POST['post']);
...
else if(isset($_POST['action']) && $_POST['action'] == "delete" && isset($_POST['post']) && !empty($_POST['post']))
{
	if(!$obj_general->hasPermission('delete',$menuId)){
		...
		$obj_mix_ink_process->updateStatus(2,$_POST['post']);
		$obj_session->data['success'] = UPDATE;
		page_redirect($obj_general->link($rout, '', '',1));
	}
}
        ...
        <section class="panel">
          <header class="panel-heading bg-white"> 
          	...
          	<span class="text-muted m-l-small pull-right">
                 ...
                 <a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus">&nbsp;&nbsp;&nbsp;</i>New Mix Ink Process </a>
           ...
           <input type="hidden" id="action" name="action" value="" />
          	<div class="table-responsive">
                <table class="table b-t text-small table-hover">
                  ...
                  <tbody>
                  <?php	
		  $total_job = $obj_mix_ink_process->getTotalInkProcess($filter_data);
...
//                      printr($limit);
		   $mix_ink_process = $obj_mix_ink_process->getInkProcess($option,$filter_data);
		  // printr($solvent_ink_process);
			  foreach($mix_ink_process as $mix_ink){ 
			 // printr($sol_ink);
                        ?>
                        <tr <?php echo ($mix_ink['status']==0) ? 'style="background-color:#FADADF" ' : '' ; ?>>
                          <td><input type="checkbox" name="post[]" value="<?php echo $mix_ink['mix_ink_p_id'];?>"></td>
                          <td><?php  echo dateformat(4, $mix_ink['date']);  ?></td>
                          <td><?php  echo $mix_ink['job_no'];  ?></td>
                          <td><?php  echo $mix_ink['job_name'];  ?></td>
                          <td><?php  echo ($mix_ink['shift']==1)?'Day':'Night';  ?></td>
                          <td><?php  echo $mix_ink['operator_name']  ?></td>
                          <td><?php  echo $mix_ink['chemist_name'];  ?></td>
                          <td>
                              
                                <div data-toggle="buttons" class="btn-group">
                                    <label class="btn btn-xs btn-success <?php  echo ($mix_ink['status']==1) ? 'active' : '';?> "> <input type="radio" 
                                     name="status" value="1" id="<?php  echo $mix_ink['mix_ink_p_id']; ?>"> <i class="fa fa-check text-active"></i>Active</label>                                   
                                    <label class="btn btn-xs btn-danger <?php  echo ($mix_ink['status']==0) ? 'active' : '';?> "> <input type="radio" 
                                        name="status" value="0" id="<?php  echo $mix_ink['mix_ink_p_id']; ?>"> <i class="fa fa-check text-active"></i>Inactive</label> 
                                </div>
                              
						  </td>
                          <td>	
                                <a href="<?php echo $obj_general->link($rout, 'mod=add&mix_ink_process='.encode($mix_ink['mix_ink_p_id']),'',1); ?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>

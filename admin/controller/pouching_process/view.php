...
if(isset($_GET['pouching_id']) && !empty($_GET['pouching_id'])){
	$pouching_id = base64_decode($_GET['pouching_id']);
		...
		$data = $obj_pouching->getPouchDetail($pouching_id);
	$slitting_details = $obj_slitting->getJobDetail($data['slitting_id']);
	$pouching_detail = $obj_pouching->view_pouching_report($pouching_id,$slitting_details);
